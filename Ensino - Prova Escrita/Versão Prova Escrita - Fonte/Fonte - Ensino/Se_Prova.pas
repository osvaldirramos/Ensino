...
type
  TForm_Cad_Prova = class(TForm)
    ...
    DTQ_Item: TDataSource;
    Panel_Abrangencia: TPanel;
    ExportQR1: TExportQR;
    Table_AvaliacaoTIPO_PROVA: TIBStringField;
    IBQuery_MontaTesteTIPO_PROVA: TIBStringField;
    Label8: TLabel;
    DBEdit1: TDBEdit;
...
uses Frame_Sel_Curso, Module, Sel_QGA, Sel_Disciplina, MontaProva,
  Editor_Texto, Rel_RelatorioDoCadastroDeAlunosComCodigo, ColetaItens,
  MontaProvaEletronica, Rel_AtaDePresenca_Cindacta2, Rel_AtaDePresenca,
  Rel_DIP, MontaGabarito, Rel_GPA_CINDACTAII, Rel_GPA, Rel_LGP,
  MontarProvaAutomatica;
...
begin
   ...
   if Table_Avaliacao.FieldByName('Tipo_Prova').AsString <> 'MISTA' then
      VS_Comando:= VS_Comando+' and PADRAO_PROVA = '+#39+Table_Avaliacao.FieldByName('Tipo_Prova').AsString+#39;
...
//   Funcoes.CorDBGrid(DBGrid_Teste, Rect, DataCol, Column, State );
   if IBQuery_MontaTeste.FieldByName( 'Status_Aplicacao').AsString = 'STANDY BY' then
      DBGrid_Teste.Canvas.Font.Color:= clRed
   else
      if IBQuery_MontaTeste.FieldByName( 'Status_Aplicacao').AsString = 'EM PAPEL' then
         DBGrid_Teste.Canvas.Font.Color:= clTeal
      else
         if IBQuery_MontaTeste.FieldByName( 'Status_Aplicacao').AsString = 'APLICAR' then
            DBGrid_Teste.Canvas.Font.Color:= $00404080
         else
            if IBQuery_MontaTeste.FieldByName( 'Status_Aplicacao').AsString = 'ANDAMENTO' then
               DBGrid_Teste.Canvas.Font.Color:= clBlack
            else
               if IBQuery_MontaTeste.FieldByName( 'Status_Aplicacao').AsString = 'DISCUSSÃO' then
                  DBGrid_Teste.Canvas.Font.Color:= clOlive
               else
                  if IBQuery_MontaTeste.FieldByName( 'Status_Aplicacao').AsString = 'ENCERRADA' then
                      DBGrid_Teste.Canvas.Font.Color:= clNavy;

   if gdSelected in State then
      DBGrid_Teste.Canvas.Brush.Color := clSkyBlue;

   DBGrid_Teste.Canvas.FillRect(Rect);
   DBGrid_Teste.DefaultDrawColumnCell(Rect, DataCol, Column, State);
end;
...
//------------------------------------------------------------------------------
// Tratamento para indicar o indice do item
//------------------------------------------------------------------------------
...
begin
   ...
   VS_Comando:= 'Select t.IdCurso, t.IdTurma, t.FaseAplicacao, t.ReferenciaAvaliacao, t.CodMateria, t.DataAplicacao, t.TotalAlunos, t.TotalItens, t.Duracao, t.CodProva, t.PesoAplicacao, t.Status_Aplicacao, t.Tipo_prova '+
                ' from  Testes_View t '+

object Form_ManualUsuario: TForm_ManualUsuario
  Left = 329
  Top = 359
  Width = 1377
  Height = 750
  Caption = 'Manual do Usu'#225'rio'
  Color = clWindow
  Ctl3D = False
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnActivate = FormActivate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 457
    Height = 711
    Align = alLeft
    BevelOuter = bvNone
    TabOrder = 0
    object Panel_Opcoes: TPanel
      Left = 0
      Top = 0
      Width = 457
      Height = 53
      Align = alTop
      BevelOuter = bvNone
      PopupMenu = PopupMenu1
      TabOrder = 0
      object JvGradient2: TJvGradient
        Left = 0
        Top = 0
        Width = 457
        Height = 53
        Align = alClient
        Style = grVertical
        StartColor = 10526880
        EndColor = 16777190
        Steps = 53
      end
      object JvShape6: TJvShape
        Left = 19
        Top = 2
        Width = 113
        Height = 47
        Brush.Color = 16378331
        Pen.Color = 33023
      end
      object JvTransparentButton_Retornar: TJvTransparentButton
        Left = 22
        Top = 3
        Width = 110
        Height = 45
        Hint = 'Retorna para janela de montagem da prova.'
        BorderWidth = 0
        Caption = 'Retornar'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        FrameStyle = fsDark
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TextAlign = ttaBottom
        OnClick = JvTransparentButton_RetornarClick
      end
      object JvShape7: TJvShape
        Left = 155
        Top = 2
        Width = 113
        Height = 47
        Brush.Color = 16378331
        Pen.Color = 33023
      end
      object JvTransparentButton_Windows: TJvTransparentButton
        Left = 157
        Top = 4
        Width = 111
        Height = 45
        Hint = 'Sai do Sistema e retorna para o Windows.'
        BorderWidth = 0
        Caption = 'Windows'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        FrameStyle = fsDark
        Glyph.Data = {
          
          290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TextAlign = ttaBottom
        OnClick = JvTransparentButton_WindowsClick
      end
    end
    object DBGrid_Providencia: TDBGrid
      Left = 0
      Top = 53
      Width = 457
      Height = 658
      Align = alClient
      Color = 7895160
      Ctl3D = False
      DataSource = DSC_Manual
      Font.Charset = ANSI_CHARSET
      Font.Color = clLime
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      Options = [dgTitles, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgCancelOnExit, dgMultiSelect]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 1
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clBlack
      TitleFont.Height = -13
      TitleFont.Name = 'Arial'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'Indice'
          Title.Alignment = taCenter
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 68
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Descricao'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Descri'#231#227'o'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 372
          Visible = True
        end>
    end
    object Panel_CadManual: TPanel
      Left = 2
      Top = 152
      Width = 449
      Height = 97
      TabOrder = 2
      object Label1: TLabel
        Left = 3
        Top = 6
        Width = 41
        Height = 13
        Caption = 'Id Indice'
      end
      object Label2: TLabel
        Left = 3
        Top = 56
        Width = 51
        Height = 13
        Caption = 'Descri'#231#227'o:'
      end
      object Label3: TLabel
        Left = 3
        Top = 80
        Width = 53
        Height = 13
        Caption = 'Modulo.....:'
      end
      object Label4: TLabel
        Left = 1
        Top = 30
        Width = 29
        Height = 13
        Caption = 'Indice'
      end
      object DBEdit_Id_Indice: TDBEdit
        Left = 59
        Top = 3
        Width = 121
        Height = 19
        DataField = 'ID_INDICE'
        DataSource = DST_Manual
        TabOrder = 0
      end
      object DBEdit_Descricao: TDBEdit
        Left = 59
        Top = 50
        Width = 386
        Height = 19
        DataField = 'DESCRICAO'
        DataSource = DST_Manual
        TabOrder = 1
      end
      object DBEdit_modulo: TDBEdit
        Left = 55
        Top = 74
        Width = 386
        Height = 19
        DataField = 'MODULO'
        DataSource = DST_Manual
        TabOrder = 2
      end
      object DBEdit_Indice: TDBEdit
        Left = 57
        Top = 27
        Width = 121
        Height = 19
        DataField = 'INDICE'
        DataSource = DST_Manual
        TabOrder = 3
      end
    end
  end
  object JvDBRichEdit_Text: TJvDBRichEdit
    Left = 457
    Top = 0
    Width = 880
    Height = 711
    DataField = 'CONTEUDO'
    DataSource = DST_Manual
    Align = alLeft
    Ctl3D = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -18
    Font.Name = 'Courier New'
    Font.Style = []
    ParentCtl3D = False
    ParentFont = False
    PopupMenu = PopupMenu1
    ScrollBars = ssVertical
    TabOrder = 1
    WantTabs = True
  end
  object PopupMenu1: TPopupMenu
    Left = 136
    Top = 328
    object Inserir1: TMenuItem
      Caption = 'Inserir'
      OnClick = Inserir1Click
    end
    object Alterar1: TMenuItem
      Caption = 'Alterar'
      OnClick = Alterar1Click
    end
    object Gravar1: TMenuItem
      Caption = 'Gravar'
      OnClick = Gravar1Click
    end
    object Cancelar1: TMenuItem
      Caption = 'Cancelar'
      OnClick = Cancelar1Click
    end
  end
  object DSC_Manual: TDataSource
    DataSet = ClientDataSet_Manual
    Left = 496
    Top = 384
  end
  object IBTable_Manual: TIBTable
    Database = DMManual.IBDatabase_Manual
    Transaction = DMManual.IBTransaction_Manual
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'INDICE'
        DataType = ftString
        Size = 15
      end
      item
        Name = 'DESCRICAO'
        DataType = ftString
        Size = 400
      end
      item
        Name = 'CONTEUDO'
        DataType = ftBlob
      end
      item
        Name = 'MODULO'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'ID_INDICE'
        DataType = ftSmallint
      end>
    IndexDefs = <
      item
        Name = 'PK_MANUALUSUARIO'
        Fields = 'INDICE'
        Options = [ixUnique]
      end>
    IndexFieldNames = 'INDICE'
    MasterFields = 'INDICE'
    MasterSource = DSC_Manual
    StoreDefs = True
    TableName = 'MANUALUSUARIO'
    Left = 536
    Top = 332
    object IBTable_ManualINDICE: TIBStringField
      FieldName = 'INDICE'
      Size = 15
    end
    object IBTable_ManualID_INDICE: TSmallintField
      FieldName = 'ID_INDICE'
    end
    object IBTable_ManualDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Size = 400
    end
    object IBTable_ManualCONTEUDO: TBlobField
      FieldName = 'CONTEUDO'
      Size = 8
    end
    object IBTable_ManualMODULO: TIBStringField
      FieldName = 'MODULO'
      Size = 100
    end
  end
  object DST_Manual: TDataSource
    DataSet = IBTable_Manual
    Left = 536
    Top = 384
  end
  object ClientDataSet_Manual: TClientDataSet
    Active = True
    Aggregates = <>
    Params = <>
    Left = 488
    Top = 328
    Data = {
      650000009619E0BD01000000180000000300000000000300000065000949645F
      496E64696365040001000000000006496E646963650100490000000100055749
      445448020002000F000944657363726963616F02004900000001000557494454
      480200020090010000}
    object ClientDataSet_ManualId_Indice: TIntegerField
      FieldName = 'Id_Indice'
    end
    object ClientDataSet_ManualIndice: TStringField
      FieldName = 'Indice'
      Size = 15
    end
    object ClientDataSet_ManualDescricao: TStringField
      FieldName = 'Descricao'
      Size = 400
    end
  end
end

object Form_RelacaoAlunosNotaBaixo: TForm_RelacaoAlunosNotaBaixo
  Left = 14
  Top = 55
  Width = 1296
  Height = 1036
  Caption = 'Form_RelacaoAlunosNotaBaixo'
  Color = 16577515
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poScreenCenter
  WindowState = wsMaximized
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object JvGradient1: TJvGradient
    Left = 1321
    Top = 0
    Width = 0
    Height = 998
    Align = alClient
    Style = grEllipse
    StartColor = 16771255
    EndColor = 16710631
    Steps = 1
  end
  object DBGrid_Alunos: TDBGrid
    Left = 157
    Top = 0
    Width = 1164
    Height = 998
    Align = alLeft
    Color = clCream
    Ctl3D = False
    DataSource = DSQ_NotaBaixa
    Options = [dgTitles, dgColLines, dgTabs, dgRowSelect, dgAlwaysShowSelection]
    ParentCtl3D = False
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'CODCURSO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Caption = 'C'#243'd. Curso'
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 88
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'TURMA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Caption = 'Turma'
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 114
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'ANO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Caption = 'Ano'
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 58
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'IDENTIDADE'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Caption = 'Identidade'
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 151
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CPF'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NOMECOMPLETO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Caption = 'Nome Completo'
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 503
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'CODPROVA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Caption = 'Prova'
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 67
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'NOTA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Caption = 'Nota'
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 84
        Visible = True
      end>
  end
  object Panel_Botoes: TPanel
    Left = 0
    Top = 0
    Width = 157
    Height = 998
    Align = alLeft
    BevelOuter = bvNone
    BorderStyle = bsSingle
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 1
    object JvGradient3: TJvGradient
      Left = 0
      Top = 0
      Width = 155
      Height = 996
      Align = alClient
      Style = grEllipse
      StartColor = 16771255
      EndColor = 16710631
      Steps = 77
    end
    object JvShape_Imprimir: TJvShape
      Left = 11
      Top = 163
      Width = 126
      Height = 49
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Imprimir: TJvTransparentButton
      Left = 13
      Top = 164
      Width = 123
      Height = 49
      Hint = 'Imprime o Relat'#243'rio'
      Caption = 'Relat'#243'rio'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_ImprimirClick
    end
    object JvShape2: TJvShape
      Left = 11
      Top = 234
      Width = 126
      Height = 49
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Retornar: TJvTransparentButton
      Left = 13
      Top = 235
      Width = 123
      Height = 49
      Hint = 'Retornar para Janela anterior.'
      Caption = 'Retornar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_RetornarClick
    end
    object JvShape3: TJvShape
      Left = 11
      Top = 290
      Width = 126
      Height = 49
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Windows: TJvTransparentButton
      Left = 13
      Top = 291
      Width = 123
      Height = 49
      Hint = 'Sai do sistema e retorna para sistema operacional.'
      Caption = 'Windows'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_WindowsClick
    end
  end
  object Query_NotaBaixa: TIBQuery
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    BufferChunks = 1000
    CachedUpdates = True
    SQL.Strings = (
      
        'Select a.identidade, a.NomeCompleto, a.CodCurso, a.Turma, a.Ano,' +
        ' n.nota, n.codprova'
      'From RELACAO_ALUNOSTURMAATIVA_VIEW a, Nota_Aluno_View n'
      'Where a.IdCurso = n.IdCurso'
      'and   a.idturma = n.IdTurma'
      'and   a.Identidade = n.Identidade'
      'and n.nota < 6 and n.nota > 0')
    Left = 195
    Top = 114
    object Query_NotaBaixaNOMECOMPLETO: TIBStringField
      FieldName = 'NOMECOMPLETO'
      Origin = 'NOTASABAIXOMEDIA_VIEW.NOMECOMPLETO'
      Size = 125
    end
    object Query_NotaBaixaCODCURSO: TIBStringField
      FieldName = 'CODCURSO'
      Origin = 'NOTASABAIXOMEDIA_VIEW.CODCURSO'
      Size = 10
    end
    object Query_NotaBaixaTURMA: TIBStringField
      FieldName = 'TURMA'
      Origin = 'NOTASABAIXOMEDIA_VIEW.TURMA'
      Size = 15
    end
    object Query_NotaBaixaANO: TIBStringField
      FieldName = 'ANO'
      Origin = 'NOTASABAIXOMEDIA_VIEW.ANO'
      Size = 4
    end
    object Query_NotaBaixaNOTA: TIBBCDField
      FieldName = 'NOTA'
      Origin = 'NOTASABAIXOMEDIA_VIEW.NOTA'
      DisplayFormat = '#0.00'
      Precision = 9
      Size = 2
    end
    object Query_NotaBaixaCODPROVA: TIBStringField
      FieldName = 'CODPROVA'
      Origin = 'NOTASABAIXOMEDIA_VIEW.CODPROVA'
      Size = 4
    end
    object Query_NotaBaixaIDENTIDADE: TIBStringField
      FieldName = 'IDENTIDADE'
      Origin = 'RELACAO_ALUNOSTURMAATIVA_VIEW.IDENTIDADE'
      FixedChar = True
    end
  end
  object DSQ_NotaBaixa: TDataSource
    Left = 240
    Top = 112
  end
end

object Form_Tab_Unidades: TForm_Tab_Unidades
  Left = 149
  Top = 57
  Width = 995
  Height = 725
  BorderIcons = []
  Caption = 'Ensino --> Unidades / SubUnidades'
  Color = 16577515
  Ctl3D = False
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 16
  object PageControl_Funcoes: TPageControl
    Left = 0
    Top = 55
    Width = 979
    Height = 623
    ActivePage = TabSheet_Unidade
    Align = alClient
    TabOrder = 0
    object TabSheet_Unidade: TTabSheet
      Caption = 'Unidade'
      object Panel1: TPanel
        Left = 0
        Top = 0
        Width = 170
        Height = 592
        Align = alLeft
        Caption = 'Panel1'
        TabOrder = 0
        object Panel4: TPanel
          Left = 1
          Top = 1
          Width = 168
          Height = 30
          Align = alTop
          Color = 16577515
          TabOrder = 0
          object Edit_BuscaSigla: TEdit
            Left = 2
            Top = 4
            Width = 140
            Height = 22
            Hint = 'Sele'#231#227'o por c'#243'digo do curso.'
            CharCase = ecUpperCase
            MaxLength = 8
            ParentShowHint = False
            ShowHint = True
            TabOrder = 0
            OnChange = Edit_BuscaSiglaChange
          end
        end
        object Panel5: TPanel
          Left = 1
          Top = 31
          Width = 168
          Height = 560
          Align = alClient
          TabOrder = 1
          object DBGrid_Unidade: TDBGrid
            Left = 1
            Top = 1
            Width = 166
            Height = 558
            Align = alClient
            DataSource = DSQ_Unidade
            Options = [dgTitles, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgAlwaysShowSelection]
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clWindowText
            TitleFont.Height = -13
            TitleFont.Name = 'MS Sans Serif'
            TitleFont.Style = []
            Columns = <
              item
                Expanded = False
                FieldName = 'IDUNIDADE'
                Title.Alignment = taCenter
                Title.Caption = 'Unidade'
                Title.Color = 16311249
                Title.Font.Charset = ANSI_CHARSET
                Title.Font.Color = clNavy
                Title.Font.Height = -13
                Title.Font.Name = 'Arial'
                Title.Font.Style = []
                Visible = True
              end>
          end
        end
      end
      object Panel_CampoUnidade: TPanel
        Left = 170
        Top = 0
        Width = 801
        Height = 592
        Align = alClient
        Color = 16577515
        TabOrder = 1
        object JvGradient3: TJvGradient
          Left = 1
          Top = 1
          Width = 799
          Height = 63
          Align = alTop
          Style = grVertical
          StartColor = 10526880
          EndColor = 16710631
          Steps = 45
        end
        object JvShape_Gravar: TJvShape
          Left = 412
          Top = 3
          Width = 114
          Height = 53
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_GravaUnid: TJvTransparentButton
          Left = 414
          Top = 4
          Width = 112
          Height = 53
          Hint = 'Grava inclus'#227'o ou altera'#231#227'o.'
          BorderWidth = 0
          Caption = 'Gravar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            2F167533146A300F6B2C136C2D1C5A260DB0855F0000}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaBottom
          OnClick = JvTransparentButton_GravaUnidClick
        end
        object JvShape_Cancelar: TJvShape
          Left = 531
          Top = 3
          Width = 114
          Height = 53
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_CancelUnid: TJvTransparentButton
          Left = 533
          Top = 4
          Width = 112
          Height = 53
          Hint = 'Cancela a inclus'#227'o ou altera'#231#227'o'
          BorderWidth = 0
          Caption = 'Cancelar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            C3C3C3C3C3C3C3C30000}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaBottom
          OnClick = JvTransparentButton_CancelUnidClick
        end
        object JvShape19: TJvShape
          Left = 11
          Top = 3
          Width = 132
          Height = 52
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_InserirUnid: TJvTransparentButton
          Left = 12
          Top = 4
          Width = 130
          Height = 52
          Hint = 'Insere uma nova unidade.'
          BorderWidth = 0
          Caption = 'Adicionar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            }
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaBottom
          OnClick = JvTransparentButton_InserirUnidClick
        end
        object JvShapeModificarProva: TJvShape
          Left = 147
          Top = 3
          Width = 132
          Height = 53
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_AlteraUnid: TJvTransparentButton
          Left = 148
          Top = 4
          Width = 130
          Height = 51
          Hint = 'Permite alterar os dados da unidade selecionado.'
          BorderWidth = 0
          Caption = 'Alterar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            FCFEFAFEF4EBFEEFE7FAFAF9F9FFF8F3F2FCFCF2FDFDFE000000}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaBottom
          OnClick = JvTransparentButton_AlteraUnidClick
        end
        object JvShape_ExcluirProva: TJvShape
          Left = 284
          Top = 3
          Width = 114
          Height = 53
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_ExcluiUnid: TJvTransparentButton
          Left = 285
          Top = 4
          Width = 112
          Height = 51
          Hint = 'Exclui a unidade selecionada.'
          BorderWidth = 0
          Caption = 'Excluir'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            7DBFBEBCFFFFFFFDFEFD}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaBottom
          OnClick = JvTransparentButton_ExcluiUnidClick
        end
        object Label_Codigo_Quadro: TLabel
          Left = 17
          Top = 102
          Width = 88
          Height = 20
          Caption = 'Unidade......:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -16
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label1: TLabel
          Left = 17
          Top = 146
          Width = 83
          Height = 20
          Caption = 'Descri'#231#227'o..:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -16
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Label4: TLabel
          Left = 17
          Top = 178
          Width = 87
          Height = 20
          Caption = 'N'#186' Fax.........:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -16
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object Edit_Unidade: TDBEdit
          Left = 113
          Top = 98
          Width = 448
          Height = 22
          CharCase = ecUpperCase
          DataField = 'IDUNIDADE'
          DataSource = DST_Unidades
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object Edit_NomeUnidade: TDBEdit
          Left = 112
          Top = 141
          Width = 649
          Height = 22
          CharCase = ecUpperCase
          DataField = 'DESCRICAO'
          DataSource = DST_Unidades
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
        end
        object DBEdit1: TDBEdit
          Left = 112
          Top = 173
          Width = 175
          Height = 22
          CharCase = ecUpperCase
          DataField = 'FAX'
          DataSource = DST_Unidades
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
        end
      end
    end
    object TabSheet_SubUnidade: TTabSheet
      Caption = 'Sub-Unidade'
      ImageIndex = 1
      object DBGrid_SubUnidade: TDBGrid
        Left = 0
        Top = 0
        Width = 169
        Height = 592
        Align = alLeft
        DataSource = DST_SubUnidade
        Options = [dgTitles, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgAlwaysShowSelection]
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -13
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'IDSUBUNIDADE'
            Title.Alignment = taCenter
            Title.Caption = 'Sub-Unidade'
            Title.Color = 16311249
            Title.Font.Charset = ANSI_CHARSET
            Title.Font.Color = clNavy
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = []
            Visible = True
          end>
      end
      object Panel_CampoSubUnidade: TPanel
        Left = 169
        Top = 0
        Width = 802
        Height = 592
        Align = alClient
        Color = 16577515
        TabOrder = 1
        object Label2: TLabel
          Left = 9
          Top = 136
          Width = 143
          Height = 16
          Caption = 'Descri'#231#227'o da Sub-Unid:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label3: TLabel
          Left = 9
          Top = 104
          Width = 143
          Height = 16
          Caption = 'Sub-Unidade....................:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label5: TLabel
          Left = 9
          Top = 162
          Width = 142
          Height = 16
          Caption = 'N'#186' Fax.................................:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object JvGradient1: TJvGradient
          Left = 1
          Top = 1
          Width = 800
          Height = 64
          Align = alTop
          Style = grVertical
          StartColor = 10526880
          EndColor = 16710631
          Steps = 45
        end
        object JvShape1: TJvShape
          Left = 412
          Top = 3
          Width = 114
          Height = 53
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_GravaSub: TJvTransparentButton
          Left = 414
          Top = 4
          Width = 112
          Height = 53
          Hint = 'Grava inclus'#227'o ou altera'#231#227'o.'
          BorderWidth = 0
          Caption = 'Gravar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            2F167533146A300F6B2C136C2D1C5A260DB0855F0000}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaBottom
          OnClick = JvTransparentButton_GravaSubClick
        end
        object JvShape2: TJvShape
          Left = 531
          Top = 3
          Width = 114
          Height = 53
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_CancelSub: TJvTransparentButton
          Left = 533
          Top = 4
          Width = 112
          Height = 53
          Hint = 'Cancela a inclus'#227'o ou altera'#231#227'o'
          BorderWidth = 0
          Caption = 'Cancelar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            C3C3C3C3C3C3C3C30000}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaBottom
          OnClick = JvTransparentButton_CancelSubClick
        end
        object JvShape3: TJvShape
          Left = 11
          Top = 3
          Width = 132
          Height = 52
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_InserirSub: TJvTransparentButton
          Left = 12
          Top = 4
          Width = 130
          Height = 52
          Hint = 'Insere uma nova Sub-Unidade.'
          BorderWidth = 0
          Caption = 'Adicionar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            }
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaBottom
          OnClick = JvTransparentButton_InserirSubClick
        end
        object JvShape5: TJvShape
          Left = 147
          Top = 3
          Width = 132
          Height = 53
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_AlteraSub: TJvTransparentButton
          Left = 148
          Top = 4
          Width = 130
          Height = 51
          Hint = 'Permite alterar os dados da sub-unidade selecionada.'
          BorderWidth = 0
          Caption = 'Alterar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            FCFEFAFEF4EBFEEFE7FAFAF9F9FFF8F3F2FCFCF2FDFDFE000000}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaBottom
          OnClick = JvTransparentButton_AlteraSubClick
        end
        object JvShape8: TJvShape
          Left = 284
          Top = 3
          Width = 114
          Height = 53
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_ExcluiSub: TJvTransparentButton
          Left = 285
          Top = 4
          Width = 112
          Height = 51
          Hint = 'Exclui a sub-unidades selecionada.'
          BorderWidth = 0
          Caption = 'Excluir'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            7DBFBEBCFFFFFFFDFEFD}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaBottom
          OnClick = JvTransparentButton_ExcluiSubClick
        end
        object DBEdit_NomeSubUnidade: TDBEdit
          Left = 157
          Top = 127
          Width = 636
          Height = 22
          CharCase = ecUpperCase
          DataField = 'DESCRICAO'
          DataSource = DST_SubUnidade
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
        end
        object DBEdit_SubUnidade: TDBEdit
          Left = 157
          Top = 97
          Width = 516
          Height = 22
          CharCase = ecUpperCase
          DataField = 'IDSUBUNIDADE'
          DataSource = DST_SubUnidade
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object DBEdit2: TDBEdit
          Left = 157
          Top = 157
          Width = 173
          Height = 22
          CharCase = ecUpperCase
          DataField = 'FAX'
          DataSource = DST_SubUnidade
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
        end
      end
    end
  end
  object Panel12: TPanel
    Left = 0
    Top = 678
    Width = 979
    Height = 8
    Align = alBottom
    BevelOuter = bvNone
    Color = 16577515
    TabOrder = 1
  end
  object Panel_Botoes: TPanel
    Left = 0
    Top = 0
    Width = 979
    Height = 55
    Align = alTop
    BevelOuter = bvNone
    Color = 16577515
    TabOrder = 2
    object JvShape4: TJvShape
      Left = 9
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Atualizar: TJvTransparentButton
      Left = 10
      Top = 3
      Width = 71
      Height = 51
      Hint = 'Atualiza a lista de posto gradua'#231#227'o'
      BorderWidth = 0
      Caption = 'Atualizar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        7C2CBE7D2CC3C3C3C3C3C3C3C3C3C3C3C300}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_AtualizarClick
    end
    object JvShape6: TJvShape
      Left = 97
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Retornar: TJvTransparentButton
      Left = 98
      Top = 3
      Width = 71
      Height = 49
      Hint = 'Retornar para Janela anterior.'
      BorderWidth = 0
      Caption = 'Retornar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_RetornarClick
    end
    object JvShape7: TJvShape
      Left = 177
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Windows: TJvTransparentButton
      Left = 178
      Top = 3
      Width = 71
      Height = 49
      Hint = 'Sai do sistema e retorna para sistema operacional.'
      BorderWidth = 0
      Caption = 'Windows'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_WindowsClick
    end
  end
  object DST_Unidades: TDataSource
    DataSet = Table_Unidades
    Left = 224
    Top = 159
  end
  object DST_SubUnidade: TDataSource
    DataSet = Table_SubUnidade
    Left = 216
    Top = 199
  end
  object DSQ_Unidade: TDataSource
    DataSet = Query_Unidade
    Left = 136
    Top = 199
  end
  object Query_Unidade: TIBQuery
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    BufferChunks = 1000
    CachedUpdates = True
    SQL.Strings = (
      'Select u.IdUnidade, u.Descricao'
      ' From Tab_Unidade u'
      ' Order by u.IdUnidade')
    Left = 137
    Top = 161
  end
  object Table_Unidades: TIBTable
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    OnPostError = Table_UnidadesPostError
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'IDUNIDADE'
        Attributes = [faRequired]
        DataType = ftString
        Size = 20
      end
      item
        Name = 'DESCRICAO'
        DataType = ftString
        Size = 130
      end
      item
        Name = 'FAX'
        DataType = ftString
        Size = 15
      end>
    IndexDefs = <
      item
        Name = 'TAB_UNIDADE_PK'
        Fields = 'IDUNIDADE'
        Options = [ixUnique]
      end>
    IndexFieldNames = 'IDUNIDADE'
    MasterFields = 'IDUNIDADE'
    MasterSource = DSQ_Unidade
    StoreDefs = True
    TableName = 'TAB_UNIDADE'
    Left = 182
    Top = 155
    object Table_UnidadesIDUNIDADE: TIBStringField
      FieldName = 'IDUNIDADE'
    end
    object Table_UnidadesDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Size = 130
    end
    object Table_UnidadesFAX: TIBStringField
      FieldName = 'FAX'
      EditMask = '(99) 9999-9999;1; '
      Size = 15
    end
  end
  object Table_SubUnidade: TIBTable
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    OnPostError = Table_SubUnidadePostError
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'IDUNIDADE'
        Attributes = [faRequired]
        DataType = ftString
        Size = 20
      end
      item
        Name = 'IDSUBUNIDADE'
        Attributes = [faRequired]
        DataType = ftString
        Size = 20
      end
      item
        Name = 'DESCRICAO'
        DataType = ftString
        Size = 130
      end
      item
        Name = 'FAX'
        DataType = ftString
        Size = 15
      end>
    IndexDefs = <
      item
        Name = 'PK_TAB_SUBUNIDADE'
        Fields = 'IDUNIDADE;IDSUBUNIDADE'
        Options = [ixUnique]
      end>
    IndexFieldNames = 'IDUNIDADE'
    MasterFields = 'IDUNIDADE'
    MasterSource = DST_Unidades
    StoreDefs = True
    TableName = 'TAB_SUBUNIDADE'
    Left = 182
    Top = 195
    object Table_SubUnidadeIDUNIDADE: TIBStringField
      FieldName = 'IDUNIDADE'
    end
    object Table_SubUnidadeIDSUBUNIDADE: TIBStringField
      FieldName = 'IDSUBUNIDADE'
    end
    object Table_SubUnidadeDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Size = 130
    end
    object Table_SubUnidadeFAX: TIBStringField
      FieldName = 'FAX'
      Size = 15
    end
  end
end

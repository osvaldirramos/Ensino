object Form_Historico: TForm_Historico
  Left = 276
  Top = 101
  Width = 1130
  Height = 822
  Caption = 
    'Ensino -> Sistema de Gerenciamento do Ensino --> Consulta Hist'#243'r' +
    'ico Geral'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  WindowState = wsMaximized
  OnActivate = FormActivate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel_Botoes: TPanel
    Left = 0
    Top = 0
    Width = 1114
    Height = 53
    Align = alTop
    BevelOuter = bvNone
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 0
    object JvGradient3: TJvGradient
      Left = 0
      Top = 0
      Width = 1114
      Height = 53
      Align = alClient
      Style = grVertical
      StartColor = 12615680
      EndColor = 16777190
      Steps = 53
    end
    object JvShape9: TJvShape
      Left = 113
      Top = 0
      Width = 66
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Fechar: TJvTransparentButton
      Left = 114
      Top = 2
      Width = 64
      Height = 51
      Hint = 'Retornar para Menu Principal'
      BorderWidth = 0
      Caption = 'Retornar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_FecharClick
    end
    object JvShape10: TJvShape
      Left = 193
      Top = 0
      Width = 69
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Windows: TJvTransparentButton
      Left = 194
      Top = 2
      Width = 67
      Height = 51
      Hint = 'Sai do sistema e retorna para sistema operacional.'
      BorderWidth = 0
      Caption = 'Windows'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_WindowsClick
    end
    object JvShape1: TJvShape
      Left = 41
      Top = 0
      Width = 66
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Excel: TJvTransparentButton
      Left = 42
      Top = 2
      Width = 64
      Height = 51
      Hint = 'Grava a alista para arquivo excel.'
      BorderWidth = 0
      Caption = 'Excel'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_ExcelClick
    end
    object Memo1: TMemo
      Left = 922
      Top = 7
      Width = 239
      Height = 33
      BevelInner = bvLowered
      Color = 5855743
      Ctl3D = False
      Lines.Strings = (
        'Informa'#231#227'o RESERVADA de acordo com a '
        'LEI N'#186' 12.527, DE 18 DE NOVEMBRO DE 2011')
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
    end
    object Panel_TotalReg: TPanel
      Left = 312
      Top = 12
      Width = 148
      Height = 33
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Total: '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
    end
  end
  object Panel_RelacaoAlunos: TPanel
    Left = 0
    Top = 53
    Width = 1114
    Height = 731
    Align = alClient
    Color = 16577515
    TabOrder = 1
    object DBGrid_Campos: TDBGrid
      Left = 1
      Top = 30
      Width = 1112
      Height = 700
      Align = alClient
      Color = 15921906
      Ctl3D = False
      DataSource = DSQ_Historico
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowSelect, dgAlwaysShowSelection, dgMultiSelect]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clMaroon
      TitleFont.Height = -13
      TitleFont.Name = 'Times New Roman'
      TitleFont.Style = []
      OnTitleClick = DBGrid_CamposTitleClick
      Columns = <
        item
          Expanded = False
          FieldName = 'CODCURSO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Curso'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TURMA'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Turma'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 81
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'ANO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Ano'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 52
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOME'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Nome Completo'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 367
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PAIS'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Pais'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 98
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'POSTOGRAD'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Posto Grad.'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 136
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QUADROESPEC'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Quadro Espec.'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 110
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'UNIDADEPER'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Unidade Pertence'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 104
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'UNIDADESER'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Unidade Serve'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 112
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'DATAINI'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = #205'nicio'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 89
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'DATATER'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'T'#233'rmino'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 86
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOTA'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Nota'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 53
          Visible = True
        end>
    end
    object ToolBar_SelecaoAluno: TToolBar
      Left = 1
      Top = 1
      Width = 1112
      Height = 29
      ButtonHeight = 24
      Caption = 'ToolBar_Selecao'
      Color = 16577515
      ParentColor = False
      TabOrder = 1
      object JvEdit_Curso: TJvEdit
        Left = 0
        Top = 2
        Width = 76
        Height = 24
        Hint = 'Realiza filtro pelo Nome de Guerra'
        Alignment = taCenter
        BevelInner = bvSpace
        BevelKind = bkFlat
        GroupIndex = -1
        MaxPixel.Font.Charset = DEFAULT_CHARSET
        MaxPixel.Font.Color = clWindowText
        MaxPixel.Font.Height = -13
        MaxPixel.Font.Name = 'MS Sans Serif'
        MaxPixel.Font.Style = []
        Modified = False
        SelStart = 0
        SelLength = 0
        AutoSelect = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        MaxLength = 10
        ParentFont = False
        ParentShowHint = False
        PasswordChar = #0
        ReadOnly = False
        ShowHint = True
        TabOrder = 4
        OnChange = JvEdit_CursoChange
      end
      object JvEdit_Turma: TJvEdit
        Left = 76
        Top = 2
        Width = 82
        Height = 24
        Hint = 'Realiza filtro pelo Identidade'
        Alignment = taCenter
        BevelInner = bvSpace
        BevelKind = bkFlat
        GroupIndex = -1
        MaxPixel.Font.Charset = DEFAULT_CHARSET
        MaxPixel.Font.Color = clWindowText
        MaxPixel.Font.Height = -13
        MaxPixel.Font.Name = 'MS Sans Serif'
        MaxPixel.Font.Style = []
        Modified = False
        SelStart = 0
        SelLength = 0
        AutoSelect = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        MaxLength = 15
        ParentFont = False
        ParentShowHint = False
        PasswordChar = #0
        ReadOnly = False
        ShowHint = True
        TabOrder = 3
        OnChange = JvEdit_CursoChange
      end
      object JvEdit_Ano: TJvEdit
        Left = 158
        Top = 2
        Width = 51
        Height = 24
        Hint = 'Realiza filtro pelo CPF.'
        Alignment = taCenter
        BevelInner = bvSpace
        BevelKind = bkFlat
        GroupIndex = -1
        MaxPixel.Font.Charset = DEFAULT_CHARSET
        MaxPixel.Font.Color = clWindowText
        MaxPixel.Font.Height = -13
        MaxPixel.Font.Name = 'MS Sans Serif'
        MaxPixel.Font.Style = []
        Modified = False
        SelStart = 0
        SelLength = 0
        AutoSelect = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        MaxLength = 4
        ParentFont = False
        ParentShowHint = False
        PasswordChar = #0
        ReadOnly = False
        ShowHint = True
        TabOrder = 2
        OnChange = JvEdit_CursoChange
      end
      object JvEdit_Nome: TJvEdit
        Left = 209
        Top = 2
        Width = 368
        Height = 24
        Hint = 'Realiza filtro pelo Nome do Instrutor'
        Alignment = taCenter
        BevelInner = bvSpace
        BevelKind = bkFlat
        GroupIndex = -1
        MaxPixel.Font.Charset = DEFAULT_CHARSET
        MaxPixel.Font.Color = clWindowText
        MaxPixel.Font.Height = -13
        MaxPixel.Font.Name = 'MS Sans Serif'
        MaxPixel.Font.Style = []
        Modified = False
        SelStart = 0
        SelLength = 0
        AutoSelect = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        MaxLength = 100
        ParentFont = False
        ParentShowHint = False
        PasswordChar = #0
        ReadOnly = False
        ShowHint = True
        TabOrder = 1
        OnChange = JvEdit_CursoChange
      end
      object JvEdit_Pais: TJvEdit
        Left = 577
        Top = 2
        Width = 101
        Height = 24
        Hint = 'Realiza filtro pelo Posto/Gradua'#231#227'o.'
        Alignment = taCenter
        BevelInner = bvSpace
        BevelKind = bkFlat
        GroupIndex = -1
        MaxPixel.Font.Charset = DEFAULT_CHARSET
        MaxPixel.Font.Color = clWindowText
        MaxPixel.Font.Height = -13
        MaxPixel.Font.Name = 'MS Sans Serif'
        MaxPixel.Font.Style = []
        Modified = False
        SelStart = 0
        SelLength = 0
        AutoSelect = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        MaxLength = 20
        ParentFont = False
        ParentShowHint = False
        PasswordChar = #0
        ReadOnly = False
        ShowHint = True
        TabOrder = 0
        OnChange = JvEdit_CursoChange
      end
      object JvEdit_PostoGrad: TJvEdit
        Left = 678
        Top = 2
        Width = 132
        Height = 24
        Hint = 'Realiza filtro pelo Posto/Gradua'#231#227'o.'
        Alignment = taCenter
        BevelInner = bvSpace
        BevelKind = bkFlat
        GroupIndex = -1
        MaxPixel.Font.Charset = DEFAULT_CHARSET
        MaxPixel.Font.Color = clWindowText
        MaxPixel.Font.Height = -13
        MaxPixel.Font.Name = 'MS Sans Serif'
        MaxPixel.Font.Style = []
        Modified = False
        SelStart = 0
        SelLength = 0
        AutoSelect = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        MaxLength = 25
        ParentFont = False
        ParentShowHint = False
        PasswordChar = #0
        ReadOnly = False
        ShowHint = True
        TabOrder = 6
        OnChange = JvEdit_CursoChange
      end
      object ToolButton1: TToolButton
        Left = 810
        Top = 2
        Width = 119
        Caption = 'ToolButton1'
        Style = tbsSeparator
      end
      object JvEdit_Unidade: TJvEdit
        Left = 929
        Top = 2
        Width = 109
        Height = 24
        Hint = 'Realiza filtro pelo Posto/Gradua'#231#227'o.'
        Alignment = taCenter
        BevelInner = bvSpace
        BevelKind = bkFlat
        GroupIndex = -1
        MaxPixel.Font.Charset = DEFAULT_CHARSET
        MaxPixel.Font.Color = clWindowText
        MaxPixel.Font.Height = -13
        MaxPixel.Font.Name = 'MS Sans Serif'
        MaxPixel.Font.Style = []
        Modified = False
        SelStart = 0
        SelLength = 0
        AutoSelect = False
        BorderStyle = bsNone
        CharCase = ecUpperCase
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        MaxLength = 25
        ParentFont = False
        ParentShowHint = False
        PasswordChar = #0
        ReadOnly = False
        ShowHint = True
        TabOrder = 5
        OnChange = JvEdit_CursoChange
      end
    end
  end
  object Query_Historico: TIBQuery
    Database = DM_Historico_APAC.IBDatabase_Historico
    Transaction = DM_Historico_APAC.IBTransaction_Historico
    BufferChunks = 1000
    CachedUpdates = True
    SQL.Strings = (
      'Select * from Historico'
      ' order by CodCurso'
      ' ')
    Left = 72
    Top = 126
    object Query_HistoricoNUMSEQ: TIntegerField
      FieldName = 'NUMSEQ'
      Origin = 'HISTORICO.NUMSEQ'
      Required = True
    end
    object Query_HistoricoIDENTIDADE: TIBStringField
      FieldName = 'IDENTIDADE'
      Origin = 'HISTORICO.IDENTIDADE'
    end
    object Query_HistoricoNOME: TIBStringField
      FieldName = 'NOME'
      Origin = 'HISTORICO.NOME'
      Size = 100
    end
    object Query_HistoricoPOSTOGRAD: TIBStringField
      FieldName = 'POSTOGRAD'
      Origin = 'HISTORICO.POSTOGRAD'
      Size = 25
    end
    object Query_HistoricoQUADROESPEC: TIBStringField
      FieldName = 'QUADROESPEC'
      Origin = 'HISTORICO.QUADROESPEC'
      Size = 8
    end
    object Query_HistoricoUNIDADEPER: TIBStringField
      FieldName = 'UNIDADEPER'
      Origin = 'HISTORICO.UNIDADEPER'
    end
    object Query_HistoricoUNIDADESER: TIBStringField
      FieldName = 'UNIDADESER'
      Origin = 'HISTORICO.UNIDADESER'
    end
    object Query_HistoricoCODCURSO: TIBStringField
      FieldName = 'CODCURSO'
      Origin = 'HISTORICO.CODCURSO'
      Size = 10
    end
    object Query_HistoricoTURMA: TIBStringField
      FieldName = 'TURMA'
      Origin = 'HISTORICO.TURMA'
      Size = 15
    end
    object Query_HistoricoANO: TIBStringField
      FieldName = 'ANO'
      Origin = 'HISTORICO.ANO'
      Size = 4
    end
    object Query_HistoricoTIPO: TIBStringField
      FieldName = 'TIPO'
      Origin = 'HISTORICO.TIPO'
      FixedChar = True
      Size = 1
    end
    object Query_HistoricoPAIS: TIBStringField
      FieldName = 'PAIS'
      Origin = 'HISTORICO.PAIS'
      Size = 12
    end
    object Query_HistoricoNOTA: TFloatField
      FieldName = 'NOTA'
      Origin = 'HISTORICO.NOTA'
      DisplayFormat = '#0.00'
      EditFormat = '#0.00'
    end
    object Query_HistoricoDATAINI: TDateTimeField
      FieldName = 'DATAINI'
      Origin = 'HISTORICO.DATAINI'
    end
    object Query_HistoricoDATATER: TDateTimeField
      FieldName = 'DATATER'
      Origin = 'HISTORICO.DATATER'
    end
    object Query_HistoricoSITUACAO: TIBStringField
      FieldName = 'SITUACAO'
      Origin = 'HISTORICO.SITUACAO'
      FixedChar = True
      Size = 1
    end
  end
  object DSQ_Historico: TDataSource
    DataSet = Query_Historico
    Left = 111
    Top = 126
  end
  object IBQuery_Executa: TIBQuery
    Database = DM_Historico_APAC.IBDatabase_Historico
    Transaction = DM_Historico_APAC.IBTransaction_Historico
    BufferChunks = 1000
    CachedUpdates = True
    SQL.Strings = (
      'Select * from Historico')
    Left = 72
    Top = 166
  end
end

...
type
  TForm_Hab_FichaAvaliacao = class(TForm)
    ...
    TabSheet_AmostraFichaAvaliacao: TTabSheet;
    ...
    JvShape4: TJvShape;
    JvTransparentButton_Providencia: TJvTransparentButton;
    ...
    Label5: TLabel;
    JvShape1: TJvShape;
    JvTransparentButton_Grava_Providencia: TJvTransparentButton;
    JvShape2: TJvShape;
    JvTransparentButton_Cancelar_Providencia: TJvTransparentButton;
    ...
    procedure PageControl_CadastroChange(Sender: TObject);
    procedure ClientDataSet_FA_AvaliacaoAfterScroll(DataSet: TDataSet);
    procedure JvTransparentButton_ProvidenciaClick(Sender: TObject);
    procedure ComboBox_SecaoSelect(Sender: TObject);
    procedure JvTransparentButton_Grava_ProvidenciaClick(Sender: TObject);
    procedure JvTransparentButton_Cancelar_ProvidenciaClick(
      Sender: TObject);
...
begin
   ...
   Panel_Providencia.Visible:= False;

   if Funcoes.GetCadCurso = 'MenuPrincipal' then
   begin

      // Alimenta o Combo de selecao da secao. Selecao dos Responsaveis

      Funcoes_TrataProvidencia.AlimentaComboSelecaoSecao(ComboBox_Secao);
      Funcoes_TrataProvidencia.AlimentaComboSelecaoResponsavel(DBComboBox_Rec_responsavel, DBComboBox_Rec_responsavel);
      ...
      PageControl_Cadastro.Visible:= true;
...
var
   ...
   bmLocal : TBookmark;
begin
   VI_Retorno:= Application.MessageBox('Essa exclusão eliminará a Disciplina selecionada '+#13+
                                       'Confirma a exclusão do registro ?', 'Atenção', MB_YesNo+mb_DefButton2+mb_IconQuestion);
   if VI_Retorno = IDYes then
   begin
      bmLocal:= ClientDataSet_FA_Avaliacao.GetBookMark;
      ...
      Funcoes_FichaAvalicaoCurso.Atualizar(VB_CorregandoFicha, WebBrowser_Providencia, Chart_Grafico, CheckBox_Msg, CheckBox_Grafico, CheckBox_Valores,
                                           IBQuery_Complemento01, IBQuery_Complemento02, IBQuery_Critica, IBQuery_Topicos, IBQuery_Itens, IBQuery_FichaAvaliacao, IBQuery_Disciplina, IBQuery_Instrutor, IBQuery_Coordenador,
                                           ProgressBar_Fichas, ProgressBar_MontaFicha, ClientDataSet_FA_Avaliacao, BarSeries_Grafico);
      ClientDataSet_FA_Avaliacao.GotoBookMark(bmLocal);
      ClientDataSet_FA_Avaliacao.FreeBookMark(bmLocal);
   end;
end;
...
begin
   if DM.IBTable_Configuracao.FieldByName('Sigla').AsString = 'CIAAR' then
      VS_MsgAvaliador:= 'Avalidor do CIAAR '+trim(IBQuery_FA_Usuario.FieldByName('PostoGraduacao').AsString)+' '+Trim(IBQuery_FA_Usuario.FieldByName('Nome').AsString)
   else
      VS_MsgAvaliador:= 'Avalidor do ICEA '+trim(IBQuery_FA_Usuario.FieldByName('PostoGraduacao').AsString)+' '+Trim(IBQuery_FA_Usuario.FieldByName('Nome').AsString);
end;
...
procedure TForm_Hab_FichaAvaliacao.PageControl_CadastroChange(Sender: TObject);
begin
   if PageControl_Cadastro.ActivePage.Caption= 'Amostra da Ficha de Avaliação' then
   begin
      VB_CorregandoFicha:= True;
      Funcoes_FichaAvalicaoCurso.Tratar_Ficha(WebBrowser_Providencia, Chart_Grafico, CheckBox_Msg, CheckBox_Grafico, CheckBox_Valores, ProgressBar_MontaFicha, ClientDataSet_FA_Avaliacao, BarSeries_Grafico,
                                              IBQuery_Complemento01, IBQuery_Complemento02, IBQuery_Critica, IBQuery_Itens, IBQuery_Topicos, IBQuery_Disciplina, IBQuery_Instrutor, IBQuery_Coordenador
...
//------------------------------------------------------------------------------
// Tratamento para habilitar a providencia
//------------------------------------------------------------------------------

procedure TForm_Hab_FichaAvaliacao.JvTransparentButton_ProvidenciaClick( Sender: TObject);
begin
    Panel_Providencia.Visible:= true;
end;

//------------------------------------------------------------------------------
// Tratamento para verificar se a providencia possui cadastro
//------------------------------------------------------------------------------

procedure TForm_Hab_FichaAvaliacao.ComboBox_SecaoSelect(Sender: TObject);
begin
   ...
   begin
      Funcoes.ExecutaQuery(IBQuery_Providencia, 'Select fp.IdCurso, fp.IdTurma, fp.IdSecao, fs.sigla, fs.descricao, '+
                                                '       fp.Rec_data as Rec_Data, fp.Rec_Responsavel , '+
                                                '       fp.Exec_data as Exec_Data, fp.Exec_Responsavel '+
                                                ' From  fp_providencias fp, fp_secao fs '+
                                                ' where fp.idcurso = :IdCurso '+
                                                ' and   fp.idturma = :IdTurma '+
                                                ' and   fS.idsecao = fp.idsecao '+
                                                ' and   fs.sigla = '+#39+ComboBox_Secao.Text+#39+
                                                ' Order By IdSecao '
                          );
      ...
      begin
         ...
         DBComboBox_Rec_responsavel.SetFocus;
      end
      else
      begin
...
//------------------------------------------------------------------------------
// Tratamento do botão para gravar o cadastro da providencia
//------------------------------------------------------------------------------

procedure TForm_Hab_FichaAvaliacao.JvTransparentButton_Grava_ProvidenciaClick(Sender: TObject);
begin
...
//------------------------------------------------------------------------------
// Tratamento do botão para cancelar o cadastro da providencia
//------------------------------------------------------------------------------
procedure TForm_Hab_FichaAvaliacao.JvTransparentButton_Cancelar_ProvidenciaClick(
  Sender: TObject);
begin
   if ((IBTable_Providencia.State = DSInsert) or (IBTable_Providencia.State = DSEdit )) then
   begin
...
procedure TForm_Hab_FichaAvaliacao.ColarClick(Sender: TObject);
begin
DBRichEdit_AcaoRecomendada.PasteFromClipboard;
end;

procedure TForm_Hab_FichaAvaliacao.Copiar1Click(Sender: TObject);
begin
//   WebBrowser_Providencia.CopyFromClipboard;
end;

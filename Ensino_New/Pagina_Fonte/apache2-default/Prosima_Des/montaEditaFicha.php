<?php
   ...
   $_SESSION[idATCONovo]=$idATCONovo;   ///////retirar
   $nSatisfatorio='N';
   $_SESSION[nome1]=$nomeAluno;	///////retirar
   
   $idCurso=$_SESSION[idCurso];
   $idTurma=$_SESSION[idTurma];
   $idFicha=1;
   $idATCO=$_SESSION[idATCO];
   $idInstrutor=$_POST[listaInstrutor];
   $idExercicio=$_SESSION[idExercicio];
   $idExercicioNovo=$_POST[exercicio];
   $idAVL=1;
   $nome=$nomeAluno;
   $data=$_POST[data];
   $local=$_POST[local];
   $fCompleta=$_POST[completa];
   $naoSatisfatorio=$nSatisfatorio;

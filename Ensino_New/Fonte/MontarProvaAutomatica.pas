...
//                                                                            //
// PROGRAMA                  : MontarProvaAutomatica.pas                      //
//                                                                            //
...
type
  ...
  TForm_MontarProvaAutomatica = class(TForm)
    ...
    procedure MaskEdit_Tot_MediasEnter(Sender: TObject);
    procedure MaskEdit_Tot_DificeisEnter(Sender: TObject);
...
begin
   ...
   JvGradient_Itens.StartColor:= Cor_Tela;
...
begin
   ...
   begin
      ...
      begin
         ...
         begin
            //--- Verifica se o item esta aplicado em uma prova recente
...
procedure TForm_MontarProvaAutomatica.MaskEdit_Tot_MediasEnter(Sender: TObject);
begin
   MaskEdit_Tot_Medias.SelectAll;
end;

procedure TForm_MontarProvaAutomatica.MaskEdit_Tot_DificeisEnter(Sender: TObject);
begin
   MaskEdit_Tot_Dificeis.SelectAll;
end;
...
var
   ...
   i, x, Prova, VI_Cont, Total_Itens: Integer;
begin
   Total_Itens:= StrToInt(Panel_Selecionadas.Caption)+1;
   Prova:= 0;
   VB_NSorteado:= True;
   While VB_NSorteado = True do
   begin
      Prova:= Random(Total_Itens);

      if ((Prova > 0 ) and (Prova < Total_Itens)) then
      begin
//         VB_NSorteado:= True;
            VB_NSorteado:= False;
...
//------------------------------------------------------------------------------
// Tratamento do Botão Homogenizar
//------------------------------------------------------------------------------
...
begin
   if ClientDataSet_Prova.FieldByName('Nivel').AsString = '' then
   begin
      ShowMessage('Verifique a Homogenização dos itens selecionados para prova.');
   end
   else
   begin
      CriarProva('EM PAPEL');
...
begin
   if ClientDataSet_Prova.FieldByName('Nivel').AsString = '' then
   begin
      ShowMessage('Verifique a Homogenizaçào dos itens selecionados para prova.');
   end
   else
   begin

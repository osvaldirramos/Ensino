...
type
  TForm_ConsultaCursosAndamento = class(TForm)
    ...
    JvCheckBox_BDS: TJvCheckBox;
    JvShape3: TJvShape;
    JvTransparentButton_BR_Office: TJvTransparentButton;
    ...
    procedure JvTransparentButton_BR_OfficeClick(Sender: TObject);
...
begin
   If ((Key = #13) and (length(MaskEdit_Ano.Text) = 4 )) then
          JvExpressButton_PesquisarClick(Sender);
...
var
   VS_Where: String;
   VI_Linha, VI_Aer, VI_EB, VI_MB, VI_PM, VI_Civis, VI_Est, VI_Homens, VI_Mulheres, VI_Deslig: Integer;

   VI_QtdAlunos, VI_TotCursos, VI_TotInst, VI_TotAlunos, VI_TotAer, VI_TotEB, VI_TotMB, VI_TotPM, VI_TotCivis, VI_TotEst, VI_TotHomens, VI_TotMulheres, VI_TotDeslig: Integer;
...
begin
   ...
   if JvCheckBox_Presencial.Checked = True then
      VS_Where:= VS_Where+' and (t.Modalidade = '+#39+'Presencial'+#39+' or t.Modalidade = '+#39+'Semi'+#39+')';
   ...
   begin
      ...
      VS_Where:= VS_Where+VS_Condicao+'( t.Turma Like '+#39+'%ACC%'+#39;
      VS_Condicao:= ' or ';
   end;
   ...
   begin
      ...
      if VS_Condicao = ' and ' then
         VS_Where:= VS_Where+VS_Condicao+'( t.Turma Like '+#39+'%APP%'+#39
      else
         VS_Where:= VS_Where+VS_Condicao+' t.Turma Like '+#39+'%APP%'+#39;
      VS_Condicao:= ' or ';
   end;
   ...
   begin
      ...
      if VS_Condicao = ' and ' then
         VS_Where:= VS_Where+VS_Condicao+'( t.Turma Like '+#39+'%TWR360%'+#39
      else
         VS_Where:= VS_Where+VS_Condicao+' t.Turma Like '+#39+'%TWR360%'+#39;
      VS_Condicao:= ' or ';
   end;
   ...
   begin
      ...
      if VS_Condicao = ' and ' then
         VS_Where:= VS_Where+VS_Condicao+'( t.Turma Like '+#39+'%TWRSICAD%'+#39
      else
         VS_Where:= VS_Where+VS_Condicao+' t.Turma Like '+#39+'%TWRSICAD%'+#39;
      VS_Condicao:= ' or ';
   end;
   ...
   begin
      ...
      if VS_Condicao = ' and ' then
         VS_Where:= VS_Where+VS_Condicao+'( t.Turma Like '+#39+'%TWR180%'+#39
      else
         VS_Where:= VS_Where+VS_Condicao+' t.Turma Like '+#39+'%TWR180%'+#39;
      VS_Condicao:= ' or ';
   end;

   if JvCheckBox_BDS.Checked = True then
   begin
      if VS_Condicao = '' then
         VS_Condicao:= ' and ';

      if VS_Condicao = ' and ' then
         VS_Where:= VS_Where+VS_Condicao+'( t.Turma Like '+#39+'BDS%'+#39
      else
         VS_Where:= VS_Where+VS_Condicao+' t.Turma Like '+#39+'BDS%'+#39;
      VS_Condicao:= ' or ';
   end;
   ...
   begin
      ...
      if JvCheckBox_Presencial.Checked = True then
         if ((IBQuery_TurmaExecutada.FieldbyName('IdCurso').AsInteger = 208) or (IBQuery_TurmaExecutada.FieldbyName('IdCurso').AsInteger = 139)) then
             VB_Gravar:= False;

      if (( JvCheckBox_Paels.Checked = True) and (JvCheckBox_BDS.Checked = False)) then
         if (Copy(IBQuery_TurmaExecutada.FieldbyName('Turma').AsString, 1, 3)  = 'BDS') then
             VB_Gravar:= False;
              ...
              if Trim(DM.IBQuery_Executa.FieldByName('Tipo_Aluno').AsString) = 'MILITAR DA AERONÁUTICA' then
                 ...
                 if Trim(DM.IBQuery_Executa.FieldByName('Tipo_Aluno').AsString) = 'MILITAR DO EXÉRCITO' Then
                    ...
                    if Trim(DM.IBQuery_Executa.FieldByName('Tipo_Aluno').AsString) = 'MILITAR DA MARINHA' Then
                       ...
                       if Trim(DM.IBQuery_Executa.FieldByName('Tipo_Aluno').AsString) = 'POLICIA MILITAR' Then
                          VI_PM:= VI_PM + 1
                       else
   ...
   StringGrid_Executados.Cells[9,0]:= ' PM';
   StringGrid_Executados.Cells[10,0]:= ' Civis';
   StringGrid_Executados.Cells[11,0]:= ' Est.';
         ...
         StringGrid_Executados.Cells[9,VI_Linha]:= Funcoes.CentraStr(IntToStr(VI_PM), 5);
         StringGrid_Executados.Cells[10,VI_Linha]:= Funcoes.CentraStr(IntToStr(VI_Civis), 5);
         StringGrid_Executados.Cells[11,VI_Linha]:= Funcoes.CentraStr(IntToStr(VI_Est), 4);
   ...
   StringGrid_Executados.Cells[9,VI_Linha]:= Funcoes.CentraStr(IntToStr(VI_TotPM), 6);
   StringGrid_Executados.Cells[10,VI_Linha]:= Funcoes.CentraStr(IntToStr(VI_TotCivis), 6);
   StringGrid_Executados.Cells[11,VI_Linha]:= Funcoes.CentraStr(IntToStr(VI_TotEst), 6);
...
begin
   ...
   Try
      ...
      MessageDlg('Verifica se existe o Excel instalado em sua máquina...',mtError, [mbOk],0);
                     ...
                     if DM.IBQuery_Executa.FieldbyName('Funcao').AsString = 'SUPERVISOR' then
                        ...
                        if DM.IBQuery_Executa.FieldbyName('Funcao').AsString = 'INSTRUTOR AUXILIAR' then
                           ...
                           if DM.IBQuery_Executa.FieldbyName('Funcao').AsString = 'COORDENADOR INSTRUTOR' then
                              ...
                              if DM.IBQuery_Executa.FieldbyName('Funcao').AsString = 'PILOTO' then
...
begin
   ...
   JvCheckBox_BDS.Visible:= Status;
end;


procedure TForm_ConsultaCursosAndamento.JvTransparentButton_BR_OfficeClick(Sender: TObject);
var
...
begin
      ...
      Sheet.getCellByPosition(0,0).setString(StringGrid_Executados.Cells[0,0]);
      Sheet.getCellByPosition(1,0).setString(StringGrid_Executados.Cells[1,0]);
      Sheet.getCellByPosition(2,0).setString(StringGrid_Executados.Cells[2,0]);
      Sheet.getCellByPosition(3,0).setString(StringGrid_Executados.Cells[3,0]);
      Sheet.getCellByPosition(4,0).setString(StringGrid_Executados.Cells[4,0]);
      Sheet.getCellByPosition(5,0).setString(StringGrid_Executados.Cells[6,0]);
      Sheet.getCellByPosition(6,0).setString(StringGrid_Executados.Cells[7,0]);
      Sheet.getCellByPosition(7,0).setString(StringGrid_Executados.Cells[8,0]);
      Sheet.getCellByPosition(8,0).setString(StringGrid_Executados.Cells[9,0]);
      Sheet.getCellByPosition(9,0).setString(StringGrid_Executados.Cells[10,0]);
      Sheet.getCellByPosition(10,0).setString(StringGrid_Executados.Cells[11,0]);
      Sheet.getCellByPosition(11,0).setString(StringGrid_Executados.Cells[13,0]);
      Sheet.getCellByPosition(12,0).setString(StringGrid_Executados.Cells[14,0]);
      Sheet.getCellByPosition(13,0).setString(StringGrid_Executados.Cells[16,0]);
      Sheet.getCellByPosition(14,0).setString( StringGrid_Executados.Cells[18,0]);
      Sheet.getCellByPosition(15,0).setString(StringGrid_Executados.Cells[19,0]);
      Sheet.getCellByPosition(16,0).setString(StringGrid_Executados.Cells[20,0]);
      Sheet.getCellByPosition(17,0).setString(StringGrid_Executados.Cells[21,0]);
      Sheet.getCellByPosition(18,0).setString(StringGrid_Executados.Cells[22,0]);
      Sheet.getCellByPosition(19,0).setString(StringGrid_Executados.Cells[23,0]);
      Sheet.getCellByPosition(20,0).setString(StringGrid_Executados.Cells[24,0]);
      Sheet.getCellByPosition(21,0).setString(StringGrid_Executados.Cells[25,0]);
      Sheet.getCellByPosition(22,0).setString(StringGrid_Executados.Cells[26,0]);
      Sheet.getCellByPosition(23,0).setString(StringGrid_Executados.Cells[27,0]);
      Sheet.getCellByPosition(24,0).setString(StringGrid_Executados.Cells[28,0]);

      For X := 0 to VI_NTurmas do
      begin
         Sheet.getCellByPosition(0,X).setString(StringGrid_Executados.Cells[0,X]);
         Sheet.getCellByPosition(1,X).setString(StringGrid_Executados.Cells[1,X]);
         Sheet.getCellByPosition(2,X).setString(StringGrid_Executados.Cells[2,X]);
         Sheet.getCellByPosition(3,X).setString(StringGrid_Executados.Cells[3,X]);
         Sheet.getCellByPosition(4,X).setString(StringGrid_Executados.Cells[4,X]);
         Sheet.getCellByPosition(5,X).setString(StringGrid_Executados.Cells[6,X]);
         Sheet.getCellByPosition(6,X).setString(StringGrid_Executados.Cells[7,X]);
         Sheet.getCellByPosition(7,X).setString(StringGrid_Executados.Cells[8,X]);
         Sheet.getCellByPosition(8,X).setString(StringGrid_Executados.Cells[9,X]);
         Sheet.getCellByPosition(9,X).setString(StringGrid_Executados.Cells[10,X]);
         Sheet.getCellByPosition(10,X).setString(StringGrid_Executados.Cells[11,X]);
         Sheet.getCellByPosition(11,X).setString(StringGrid_Executados.Cells[13,X]);
         Sheet.getCellByPosition(12,X).setString(StringGrid_Executados.Cells[14,X]);
         Sheet.getCellByPosition(13,X).setString(StringGrid_Executados.Cells[16,X]);
         Sheet.getCellByPosition(14,X).setString( StringGrid_Executados.Cells[18,X]);
         Sheet.getCellByPosition(15,X).setString(StringGrid_Executados.Cells[19,X]);
         Sheet.getCellByPosition(16,X).setString(StringGrid_Executados.Cells[20,X]);
         Sheet.getCellByPosition(17,X).setString(StringGrid_Executados.Cells[21,X]);
         Sheet.getCellByPosition(18,X).setString(StringGrid_Executados.Cells[22,X]);
         Sheet.getCellByPosition(19,X).setString(StringGrid_Executados.Cells[23,X]);
         Sheet.getCellByPosition(20,X).setString(StringGrid_Executados.Cells[24,X]);
         Sheet.getCellByPosition(21,X).setString(StringGrid_Executados.Cells[25,X]);
         Sheet.getCellByPosition(22,X).setString(StringGrid_Executados.Cells[26,X]);
         Sheet.getCellByPosition(23,X).setString(StringGrid_Executados.Cells[27,X]);
         Sheet.getCellByPosition(24,X).setString(StringGrid_Executados.Cells[28,X]);
      end;

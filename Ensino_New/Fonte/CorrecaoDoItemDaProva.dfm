object Form_CorrecaoDoItemDaProvaSistema: TForm_CorrecaoDoItemDaProvaSistema
  Left = 0
  Top = 103
  Width = 1263
  Height = 714
  Caption = 'Ensino --> Corre'#231#227'o de Irregularidades da Prova'
  Color = 16577515
  Ctl3D = False
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 16
  object Panel_TratamentoDoItem: TPanel
    Left = 185
    Top = 0
    Width = 1060
    Height = 673
    Align = alClient
    Caption = 'Panel2'
    TabOrder = 0
    object Panel_Alternativa: TPanel
      Left = 1
      Top = 1
      Width = 377
      Height = 671
      Align = alLeft
      Caption = 'Panel_Alternativa'
      TabOrder = 0
      object DBGrid_Query_Item_Prova: TDBGrid
        Left = 1
        Top = 28
        Width = 375
        Height = 642
        Align = alClient
        Color = 14544639
        Ctl3D = False
        DataSource = Form_Cad_CorrecaoProva.DSQ_ItemTeste
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -18
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgAlwaysShowSelection]
        ParentCtl3D = False
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = ANSI_CHARSET
        TitleFont.Color = clRed
        TitleFont.Height = -13
        TitleFont.Name = 'Times New Roman'
        TitleFont.Style = [fsBold]
        Columns = <
          item
            Alignment = taCenter
            Color = 10079436
            Expanded = False
            FieldName = 'NUMEROQUESTAO'
            Font.Charset = ANSI_CHARSET
            Font.Color = clNavy
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Quest'#227'o'
            Title.Color = 16311249
            Title.Font.Charset = ANSI_CHARSET
            Title.Font.Color = clNavy
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = []
            Width = 57
            Visible = True
          end
          item
            Alignment = taCenter
            Color = 10079436
            Expanded = False
            FieldName = 'NUMITEM'
            Font.Charset = ANSI_CHARSET
            Font.Color = clNavy
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'N'#186' Item'
            Title.Color = 16311249
            Title.Font.Charset = ANSI_CHARSET
            Title.Font.Color = clNavy
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = []
            Visible = True
          end
          item
            Alignment = taCenter
            Color = 10079436
            Expanded = False
            FieldName = 'QUESTAOANULADA'
            Font.Charset = ANSI_CHARSET
            Font.Color = clNavy
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Anulada'
            Title.Color = 16311249
            Title.Font.Charset = ANSI_CHARSET
            Title.Font.Color = clNavy
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = []
            Width = 55
            Visible = True
          end
          item
            Alignment = taCenter
            Color = 10079436
            Expanded = False
            FieldName = 'QUESTAOIRREGULAR'
            Font.Charset = ANSI_CHARSET
            Font.Color = clNavy
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'Irregular'
            Title.Color = 16311249
            Title.Font.Charset = ANSI_CHARSET
            Title.Font.Color = clNavy
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = []
            Width = 52
            Visible = True
          end
          item
            Alignment = taCenter
            Color = 10079436
            Expanded = False
            FieldName = 'ALTERNATIVACORRETAA'
            Font.Charset = ANSI_CHARSET
            Font.Color = clNavy
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'A'
            Title.Color = 16311249
            Title.Font.Charset = ANSI_CHARSET
            Title.Font.Color = clNavy
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = []
            Width = 27
            Visible = True
          end
          item
            Alignment = taCenter
            Color = 10079436
            Expanded = False
            FieldName = 'ALTERNATIVACORRETAB'
            Font.Charset = ANSI_CHARSET
            Font.Color = clNavy
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'B'
            Title.Color = 16311249
            Title.Font.Charset = ANSI_CHARSET
            Title.Font.Color = clNavy
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = []
            Width = 27
            Visible = True
          end
          item
            Alignment = taCenter
            Color = 10079436
            Expanded = False
            FieldName = 'ALTERNATIVACORRETAC'
            Font.Charset = ANSI_CHARSET
            Font.Color = clNavy
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'C'
            Title.Color = 16311249
            Title.Font.Charset = ANSI_CHARSET
            Title.Font.Color = clNavy
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = []
            Width = 28
            Visible = True
          end
          item
            Alignment = taCenter
            Color = 10079436
            Expanded = False
            FieldName = 'ALTERNATIVACORRETAD'
            Font.Charset = ANSI_CHARSET
            Font.Color = clNavy
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Title.Alignment = taCenter
            Title.Caption = 'D'
            Title.Color = 16311249
            Title.Font.Charset = ANSI_CHARSET
            Title.Font.Color = clNavy
            Title.Font.Height = -13
            Title.Font.Name = 'Arial'
            Title.Font.Style = []
            Width = 27
            Visible = True
          end>
      end
      object Panel5: TPanel
        Left = 1
        Top = 1
        Width = 375
        Height = 27
        Align = alTop
        BevelOuter = bvNone
        BorderStyle = bsSingle
        Color = 16577515
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 1
        object Panel3: TPanel
          Left = 132
          Top = -1
          Width = 110
          Height = 27
          BevelOuter = bvNone
          BorderStyle = bsSingle
          Caption = 'Quest'#227'o'
          Color = 16311249
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object Panel2: TPanel
          Left = 241
          Top = -1
          Width = 114
          Height = 27
          BevelOuter = bvNone
          BorderStyle = bsSingle
          Caption = 'Alternativa Correta'
          Color = 16311249
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
        end
      end
    end
    object GroupBox_TratamentoIrregularidade: TGroupBox
      Left = 378
      Top = 1
      Width = 681
      Height = 671
      Align = alClient
      Caption = ' Tratamento de Irregularidade no Item '
      Color = 16577515
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      TabOrder = 1
      object GroupBox_Questao: TGroupBox
        Left = 1
        Top = 18
        Width = 679
        Height = 57
        Align = alTop
        Caption = ' Quest'#227'o '
        TabOrder = 0
        object JvShape6: TJvShape
          Left = 23
          Top = 17
          Width = 95
          Height = 38
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_Anulada: TJvTransparentButton
          Left = 24
          Top = 19
          Width = 93
          Height = 35
          Hint = 'Indica que a alternativa foi anulada'
          Caption = 'Anulada'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaRight
          OnClick = JvTransparentButton_AnuladaClick
        end
        object JvShape7: TJvShape
          Left = 129
          Top = 17
          Width = 95
          Height = 38
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_Irregular: TJvTransparentButton
          Left = 130
          Top = 19
          Width = 93
          Height = 35
          Hint = 'Indica que a alternativa '#233' Irregular'
          Caption = 'Irregular'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaRight
          OnClick = JvTransparentButton_IrregularClick
        end
      end
      object GroupBox_AlternativaCorreta: TGroupBox
        Left = 1
        Top = 75
        Width = 679
        Height = 57
        Align = alTop
        Caption = ' Alternativa Correta '
        TabOrder = 1
        object JvShape2: TJvShape
          Left = 17
          Top = 19
          Width = 55
          Height = 33
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_A: TJvTransparentButton
          Left = 18
          Top = 21
          Width = 53
          Height = 30
          Hint = 'Indica que a alternativa Correta '#233' a A'
          Caption = 'A   '
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            F3F3F3000000}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaRight
          OnClick = JvTransparentButton_AClick
        end
        object JvShape3: TJvShape
          Left = 81
          Top = 19
          Width = 55
          Height = 33
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_B: TJvTransparentButton
          Left = 82
          Top = 21
          Width = 53
          Height = 30
          Hint = 'Indica que a alternativa Correta '#233' a B'
          Caption = 'B   '
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            F3F3F3000000}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaRight
          OnClick = JvTransparentButton_BClick
        end
        object JvShape4: TJvShape
          Left = 145
          Top = 19
          Width = 55
          Height = 33
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_C: TJvTransparentButton
          Left = 146
          Top = 21
          Width = 53
          Height = 30
          Hint = 'Indica que a alternativa Correta '#233' a C'
          Caption = 'C   '
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            F3F3F3000000}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaRight
          OnClick = JvTransparentButton_CClick
        end
        object JvShape5: TJvShape
          Left = 209
          Top = 19
          Width = 55
          Height = 33
          Brush.Color = 16378331
          Pen.Color = 33023
        end
        object JvTransparentButton_D: TJvTransparentButton
          Left = 210
          Top = 21
          Width = 53
          Height = 30
          Hint = 'Indica que a alternativa Correta '#233' a D'
          Caption = 'D   '
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          FrameStyle = fsDark
          Glyph.Data = {
            
            F3F3F3000000}
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TextAlign = ttaRight
          OnClick = JvTransparentButton_DClick
        end
      end
      object GroupBox_DescricaoDoItem: TGroupBox
        Left = 1
        Top = 132
        Width = 679
        Height = 354
        Align = alClient
        Caption = '< Descri'#231#227'o do Item>'
        TabOrder = 2
        object JvDBRichEdit_Text: TJvDBRichEdit
          Left = 1
          Top = 18
          Width = 677
          Height = 335
          DataField = 'DESCENALT'
          DataSource = Form_Cad_CorrecaoProva.DST_Item_Desc
          Align = alClient
          Color = 14544639
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -18
          Font.Name = 'Courier New'
          Font.Style = []
          ParentFont = False
          ScrollBars = ssVertical
          TabOrder = 0
        end
      end
      object GroupBox_TrataObservacaoDoItem: TGroupBox
        Left = 1
        Top = 486
        Width = 679
        Height = 184
        Align = alBottom
        Caption = ' Historico do Item '
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        object DBRichEdit_Observacao: TDBRichEdit
          Left = 1
          Top = 18
          Width = 677
          Height = 165
          Align = alClient
          Ctl3D = False
          DataField = 'OBSERVACAO'
          DataSource = Form_Cad_CorrecaoProva.DST_Item_Desc
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Arial'
          Font.Style = []
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 0
        end
      end
    end
  end
  object Panel_BotoesIniciais: TPanel
    Left = 0
    Top = 0
    Width = 185
    Height = 673
    Align = alLeft
    BevelOuter = bvNone
    BorderStyle = bsSingle
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 1
    object JvGradient4: TJvGradient
      Left = 0
      Top = 49
      Width = 183
      Height = 622
      Align = alClient
      Style = grEllipse
      StartColor = 16773606
      EndColor = 4227200
      Steps = 73
    end
    object JvShape_FinalizaCorrecao: TJvShape
      Left = 3
      Top = 231
      Width = 176
      Height = 63
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_FinalizaCorrecao: TJvTransparentButton
      Left = 4
      Top = 232
      Width = 174
      Height = 61
      Hint = 'Sai do sistema e retorna para sistema operacional.'
      Caption = 'Windows'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_FinalizaCorrecaoClick
    end
    object JvShape1: TJvShape
      Left = 3
      Top = 163
      Width = 176
      Height = 63
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Fechar: TJvTransparentButton
      Left = 4
      Top = 164
      Width = 174
      Height = 61
      Hint = 'Fecha a janela e retorna para corre'#231#227'o da Prova'
      Caption = 'Fechar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_FecharClick
    end
    object Label_Prova: TLabel
      Left = 1
      Top = 82
      Width = 60
      Height = 17
      Caption = 'Respostas'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Times New Roman'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Memo2: TMemo
      Left = 0
      Top = 0
      Width = 183
      Height = 49
      Align = alTop
      BevelInner = bvNone
      BevelOuter = bvNone
      BorderStyle = bsNone
      Color = 5855743
      Ctl3D = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial Narrow'
      Font.Style = []
      Lines.Strings = (
        'Informa'#231#227'o RESERVADA de acordo '
        'com a LEI N'#186' 12.527, de 18 de'
        'Novembro de 2011')
      ParentCtl3D = False
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
    end
    object Documento: TJvDBRichEdit
      Left = -1827
      Top = 338
      Width = 738
      Height = 321
      DataField = 'DESCRICAO'
      TabOrder = 1
    end
  end
end

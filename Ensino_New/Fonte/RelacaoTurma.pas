...
type
  TForm_RelacaoTurma = class(TForm)
    ...
    CheckBox_LPNA: TCheckBox;

    procedure Tratar_RelAvaliacao;
...
uses Sel_Turma, Module, MenuPrincipal;
...
begin
   ...
   VS_Comando:= 'Select a.SARAM, a.NomeGuerra, a.Identidade, a.NomeCompleto, a.CPF, a.IdUnidade, a.IdSubUnidade, a.NotaFinal, p.Email, P.LPNA'+
   ...
   begin
       ...
       CheckBox_LPNA.Visible:= True;
   end
   else
   begin
      CheckBox_Nota.Visible:= False;
      CheckBox_LPNA.Visible:= False;
   end;
   if (((Form_MenuPrincipal.DBText_SecaoUsuario.Caption = 'EAVA') or (Funcoes.GetSecao = 'ADMINISTRAÇÃO')) and
        (DM.IBTable_Configuracao.FieldByName('SIGLA').AsString = 'ICEA')) Then
      Tratar_RelAvaliacao
   else
...
begin
   ...
   if CheckBox_LPNA.Checked = True then
      VS_Linha:= VS_Linha+'LPNA                 ';
   ...
   begin
      ...
      if CheckBox_NomeGuerra.Checked = True then
       ...
       if CheckBox_LPNA.Checked = True then
         VS_Linha:= VS_Linha+Funcoes.ReplStr(IBQuery_AlunoTurma.FieldByName('LPNA').AsString,' ', 21);

       if CheckBox_Nota.Checked = True then
         VS_Linha:= VS_Linha+' '+FormatFloat('#0.00', IBQuery_AlunoTurma.FieldByName('NotaFinal').AsFloat);

       if CheckBox_EMail.Checked = True then
         VS_Linha:= VS_Linha+IBQuery_AlunoTurma.FieldByName('LPNA').AsString;
...
//------------------------------------------------------------------------------
// Tratar relacao da avaliacao
//------------------------------------------------------------------------------

procedure TForm_RelacaoTurma.Tratar_RelAvaliacao;
var
   VS_Linha: String;
begin
   Text1.Lines.Clear;
   ...
   //---- Tratamento do Cabecalho
   VS_Linha:= Funcoes.ReplStr('Nome Guerra',' ', 18)+
                 Funcoes.ReplStr('NomeCompleto',' ', 50)+
                 Funcoes.ReplStr('CPF', ' ',22)+
                 Funcoes.ReplStr('Nota', ' ',6)+
                 'E-Mail';
   ...
   begin
      ...
      VS_Linha:= VS_Linha+Funcoes.ReplStr(IBQuery_AlunoTurma.FieldByName('NomeGuerra').AsString,' ', 18);
      VS_Linha:= VS_Linha+Funcoes.ReplStr(copy(IBQuery_AlunoTurma.FieldByName('NomeCompleto').AsString,1, 49),' ', 50);
      VS_Linha:= VS_Linha+Funcoes.ReplStr(Copy(IBQuery_AlunoTurma.FieldByName('CPF').AsString, 1,6)+'-'+
                                          Copy(IBQuery_AlunoTurma.FieldByName('CPF').AsString, 7,5) , ' ', 22);
      VS_Linha:= VS_Linha+Funcoes.ReplStr(FormatFloat('#0.00', IBQuery_AlunoTurma.FieldByName('NotaFinal').AsFloat), ' ', 6);
      VS_Linha:= VS_Linha+IBQuery_AlunoTurma.FieldByName('EMail').AsString;

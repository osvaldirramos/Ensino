object Form_Estatistica_Diarias: TForm_Estatistica_Diarias
  Left = 370
  Top = 124
  Width = 774
  Height = 640
  Caption = 'Sistema de Gerenciamento do Ensino - Estatistica Di'#225'rias'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnActivate = FormActivate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel_Botoes: TPanel
    Left = 0
    Top = 0
    Width = 171
    Height = 609
    Align = alLeft
    BevelOuter = bvNone
    TabOrder = 0
    object JvGradient3: TJvGradient
      Left = 0
      Top = 0
      Width = 171
      Height = 609
      Align = alClient
      Style = grVertical
      StartColor = 4227200
      EndColor = 13882323
      Steps = 120
    end
    object JvShape4: TJvShape
      Left = 8
      Top = 196
      Width = 155
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Imprimir: TJvTransparentButton
      Left = 7
      Top = 197
      Width = 152
      Height = 53
      Hint = 'Imprime o relat'#243'rio o controle geral de graus.'
      BorderWidth = 0
      Caption = 'Excel'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_ImprimirClick
    end
    object JvShape6: TJvShape
      Left = 8
      Top = 258
      Width = 155
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Retornar: TJvTransparentButton
      Left = 7
      Top = 259
      Width = 152
      Height = 51
      Hint = 'Retorna para janela de montagem da prova.'
      BorderWidth = 0
      Caption = 'Retornar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_RetornarClick
    end
    object JvShape1: TJvShape
      Left = 8
      Top = 321
      Width = 155
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Windows: TJvTransparentButton
      Left = 7
      Top = 323
      Width = 152
      Height = 51
      Hint = 'Sai do Sistema e retorna para o Windows.'
      BorderWidth = 0
      Caption = 'Windows'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_WindowsClick
    end
    object Label3: TLabel
      Left = 10
      Top = 55
      Width = 107
      Height = 16
      Caption = 'Sele'#231#227'o por Curso'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label_Ano: TLabel
      Left = 12
      Top = 10
      Width = 23
      Height = 16
      Caption = 'Ano'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object ComboBox_Curso: TComboBox
      Left = 8
      Top = 75
      Width = 112
      Height = 24
      BevelInner = bvSpace
      BevelKind = bkFlat
      CharCase = ecUpperCase
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 0
      OnChange = ComboBox_CursoChange
    end
    object MaskEdit_Ano: TMaskEdit
      Left = 10
      Top = 27
      Width = 57
      Height = 22
      BevelInner = bvNone
      BevelOuter = bvRaised
      CharCase = ecUpperCase
      Ctl3D = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 1
      OnChange = MaskEdit_AnoChange
    end
    object GroupBox_QtdCursos: TGroupBox
      Left = 5
      Top = 111
      Width = 159
      Height = 55
      Caption = ' Totaliza'#231#227'o '
      Color = 12902881
      ParentColor = False
      TabOrder = 2
      object Label_QtdCursos: TLabel
        Left = 5
        Top = 21
        Width = 92
        Height = 13
        Alignment = taCenter
        AutoSize = False
        Caption = 'Qtd Cursos'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
    end
  end
  object Panel1: TPanel
    Left = 171
    Top = 0
    Width = 595
    Height = 609
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 1
    object Panel_TratarInstrutor: TPanel
      Left = 0
      Top = 0
      Width = 595
      Height = 19
      Align = alTop
      BevelOuter = bvNone
      Color = 16311249
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 0
      object Edit_Instrutor: TEdit
        Left = 0
        Top = 0
        Width = 392
        Height = 19
        BevelInner = bvNone
        BevelOuter = bvRaised
        CharCase = ecUpperCase
        MaxLength = 100
        TabOrder = 0
      end
    end
    object DBGrid_SelecionaInstrutor: TDBGrid
      Left = 0
      Top = 19
      Width = 595
      Height = 590
      Align = alClient
      Color = 15921906
      Ctl3D = False
      DataSource = DSQ_Estatistica
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgCancelOnExit, dgMultiSelect]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 1
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clBlack
      TitleFont.Height = -13
      TitleFont.Name = 'Arial'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'NOMECOMPLETO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Nome Completo'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 388
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DATACHEGADA'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Chegada'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 92
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DATASAIDA'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Sa'#237'da'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 84
          Visible = True
        end>
    end
  end
  object IBQuery_Estatistica: TIBQuery
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    BufferChunks = 1000
    CachedUpdates = False
    DataSource = DM.DSQ_MontaTurma
    SQL.Strings = (
      'select distinct i.NomeCompleto, i.datachegada, i.datasaida'
      'From InstrutorTurma i, Curso c'
      'Where i.IdCurso = c.IdCurso'
      'and c.CodCurso = '#39'PROSIMA'#39
      'order by  NomeCompleto')
    Left = 67
    Top = 330
    object IBQuery_EstatisticaNOMECOMPLETO: TIBStringField
      FieldName = 'NOMECOMPLETO'
      Origin = 'INSTRUTORTURMA.NOMECOMPLETO'
      Size = 125
    end
    object IBQuery_EstatisticaDATACHEGADA: TDateTimeField
      FieldName = 'DATACHEGADA'
      Origin = 'INSTRUTORTURMA.DATACHEGADA'
    end
    object IBQuery_EstatisticaDATASAIDA: TDateTimeField
      FieldName = 'DATASAIDA'
      Origin = 'INSTRUTORTURMA.DATASAIDA'
    end
  end
  object DSQ_Estatistica: TDataSource
    DataSet = IBQuery_Estatistica
    Left = 112
    Top = 328
  end
end

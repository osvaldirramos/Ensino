object Form_Tab_Paises: TForm_Tab_Paises
  Left = 294
  Top = 32
  Width = 726
  Height = 792
  Caption = 'Ensino --> Pa'#237'ses'
  Color = 4227200
  Ctl3D = False
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 16
  object Panel_Entrada_Dados: TPanel
    Left = 0
    Top = 118
    Width = 718
    Height = 74
    Align = alTop
    Color = 16577515
    TabOrder = 0
    object Label_Unidade: TLabel
      Left = 17
      Top = 47
      Width = 89
      Height = 16
      Caption = 'Descri'#231#227'o........:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label_Nome: TLabel
      Left = 17
      Top = 15
      Width = 91
      Height = 16
      Caption = 'Nome Pais.......:'
      FocusControl = DBEdit_Descricao
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object JvEnterAsTab1: TJvEnterAsTab
      Left = 408
      Top = 1
      Width = 28
      Height = 28
    end
    object DBEdit_Nome: TDBEdit
      Left = 114
      Top = 9
      Width = 148
      Height = 24
      BevelInner = bvSpace
      BevelKind = bkFlat
      BorderStyle = bsNone
      CharCase = ecUpperCase
      DataField = 'NOMEPAIS'
      DataSource = DST_Cadastro
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object DBEdit_Descricao: TDBEdit
      Left = 114
      Top = 42
      Width = 559
      Height = 24
      BevelInner = bvSpace
      BevelKind = bkFlat
      BorderStyle = bsNone
      CharCase = ecUpperCase
      DataField = 'DESCPAIS'
      DataSource = DST_Cadastro
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
    end
  end
  object Panel_Grid: TPanel
    Left = 0
    Top = 192
    Width = 718
    Height = 569
    Align = alClient
    BevelInner = bvLowered
    BorderWidth = 4
    Color = 16577515
    TabOrder = 1
    object DBGrid_Campos: TDBGrid
      Left = 6
      Top = 35
      Width = 707
      Height = 528
      Align = alLeft
      DataSource = DSQ_Consulta
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowSelect, dgAlwaysShowSelection, dgMultiSelect]
      ParentFont = False
      TabOrder = 0
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clMaroon
      TitleFont.Height = -13
      TitleFont.Name = 'Times New Roman'
      TitleFont.Style = []
      OnTitleClick = DBGrid_CamposTitleClick
      Columns = <
        item
          Expanded = False
          FieldName = 'NOMEPAIS'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Pa'#237's'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 216
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DESCPAIS'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Descri'#231#227'o'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 460
          Visible = True
        end>
    end
    object ToolBar1: TToolBar
      Left = 6
      Top = 6
      Width = 706
      Height = 29
      ButtonHeight = 24
      Caption = 'ToolBar_Selecao'
      TabOrder = 1
      object Edit_Nome: TEdit
        Left = 0
        Top = 2
        Width = 217
        Height = 24
        BevelInner = bvSpace
        BevelKind = bkFlat
        BorderStyle = bsNone
        CharCase = ecUpperCase
        MaxLength = 12
        TabOrder = 1
        OnChange = Edit_NomeChange
      end
      object Edit_Descricao: TEdit
        Left = 217
        Top = 2
        Width = 464
        Height = 24
        BevelInner = bvSpace
        BevelKind = bkFlat
        BorderStyle = bsNone
        CharCase = ecUpperCase
        MaxLength = 40
        TabOrder = 0
        OnChange = Edit_DescricaoChange
      end
    end
  end
  object Panel_Botoes: TPanel
    Left = 0
    Top = 0
    Width = 718
    Height = 55
    Align = alTop
    BevelOuter = bvNone
    Color = 4227200
    TabOrder = 2
    object JvShape4: TJvShape
      Left = 9
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Atualizar: TJvTransparentButton
      Left = 10
      Top = 3
      Width = 71
      Height = 51
      Hint = 'Atualiza a lista de quadros'
      BorderWidth = 0
      Caption = 'Atualizar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        7C2CBE7D2CC3C3C3C3C3C3C3C3C3C3C3C300}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_AtualizarClick
    end
    object JvShape6: TJvShape
      Left = 97
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Retornar: TJvTransparentButton
      Left = 98
      Top = 3
      Width = 71
      Height = 49
      Hint = 'Retornar para Janela anterior.'
      BorderWidth = 0
      Caption = 'Retornar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_RetornarClick
    end
    object JvShape7: TJvShape
      Left = 177
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Windows: TJvTransparentButton
      Left = 178
      Top = 3
      Width = 71
      Height = 49
      Hint = 'Sai do sistema e retorna para sistema operacional.'
      BorderWidth = 0
      Caption = 'Windows'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_WindowsClick
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 55
    Width = 718
    Height = 63
    Align = alTop
    BevelOuter = bvNone
    BorderStyle = bsSingle
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 3
    object JvGradient1: TJvGradient
      Left = 0
      Top = 0
      Width = 716
      Height = 61
      Align = alClient
      Style = grVertical
      StartColor = 4227200
      EndColor = 16710631
      Steps = 45
    end
    object JvShape1: TJvShape
      Left = 412
      Top = 3
      Width = 114
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Grava: TJvTransparentButton
      Left = 414
      Top = 4
      Width = 112
      Height = 53
      Hint = 'Grava inclus'#227'o ou altera'#231#227'o.'
      BorderWidth = 0
      Caption = 'Gravar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        2F167533146A300F6B2C136C2D1C5A260DB0855F0000}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_GravaClick
    end
    object JvShape2: TJvShape
      Left = 531
      Top = 3
      Width = 114
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Cancel: TJvTransparentButton
      Left = 533
      Top = 4
      Width = 112
      Height = 53
      Hint = 'Cancela a inclus'#227'o ou altera'#231#227'o'
      BorderWidth = 0
      Caption = 'Cancelar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        C3C3C3C3C3C3C3C30000}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_CancelClick
    end
    object JvShape3: TJvShape
      Left = 11
      Top = 3
      Width = 132
      Height = 52
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Inserir: TJvTransparentButton
      Left = 12
      Top = 4
      Width = 130
      Height = 52
      Hint = 'Insere um novo Pais'
      BorderWidth = 0
      Caption = 'Adicionar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        }
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_InserirClick
    end
    object JvShape5: TJvShape
      Left = 147
      Top = 3
      Width = 132
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Altera: TJvTransparentButton
      Left = 148
      Top = 4
      Width = 130
      Height = 51
      Hint = 'Permite alterar os dados do pais selecionado'
      BorderWidth = 0
      Caption = 'Alterar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FCFEFAFEF4EBFEEFE7FAFAF9F9FFF8F3F2FCFCF2FDFDFE000000}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_AlteraClick
    end
    object JvShape8: TJvShape
      Left = 284
      Top = 3
      Width = 114
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Exclui: TJvTransparentButton
      Left = 285
      Top = 4
      Width = 112
      Height = 51
      Hint = 'Exclui o pais selecionado.'
      BorderWidth = 0
      Caption = 'Excluir'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        7DBFBEBCFFFFFFFDFEFD}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_ExcluiClick
    end
  end
  object DST_Cadastro: TDataSource
    DataSet = Table_Cadastro
    Left = 319
    Top = 206
  end
  object DSQ_Consulta: TDataSource
    DataSet = Query_Consulta
    Left = 263
    Top = 206
  end
  object Query_Consulta: TIBQuery
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    BufferChunks = 1000
    CachedUpdates = True
    SQL.Strings = (
      'Select  NomePais, DescPais'
      'From Tab_Pais'
      'Order By NomePais')
    Left = 264
    Top = 169
    object Query_ConsultaNOMEPAIS: TIBStringField
      FieldName = 'NOMEPAIS'
      Origin = 'TAB_PAIS.NOMEPAIS'
      Required = True
      Size = 12
    end
    object Query_ConsultaDESCPAIS: TIBStringField
      FieldName = 'DESCPAIS'
      Origin = 'TAB_PAIS.DESCPAIS'
      Required = True
      Size = 40
    end
  end
  object Table_Cadastro: TIBTable
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    OnPostError = Table_CadastroPostError
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'NOMEPAIS'
        Attributes = [faRequired]
        DataType = ftString
        Size = 12
      end
      item
        Name = 'DESCPAIS'
        Attributes = [faRequired]
        DataType = ftString
        Size = 40
      end>
    IndexDefs = <
      item
        Name = 'IDTAB_PAIS_PK'
        Fields = 'NOMEPAIS'
        Options = [ixUnique]
      end>
    IndexFieldNames = 'NOMEPAIS'
    MasterFields = 'NOMEPAIS'
    MasterSource = DSQ_Consulta
    StoreDefs = True
    TableName = 'TAB_PAIS'
    Left = 320
    Top = 169
  end
end

object Form_Tab_Cargos: TForm_Tab_Cargos
  Left = 266
  Top = 137
  Width = 878
  Height = 784
  Caption = 'Ensino --> Sistema de Gerenciamento do Ensino --> Cargos'
  Color = 16577515
  Ctl3D = False
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 16
  object JvGradient1: TJvGradient
    Left = 0
    Top = 267
    Width = 870
    Height = 486
    Align = alClient
    Style = grVertical
    StartColor = 16771255
    EndColor = 16710631
    Steps = 45
  end
  object Panel_Entrada_Dados: TPanel
    Left = 0
    Top = 118
    Width = 870
    Height = 149
    Align = alTop
    Color = 16577515
    TabOrder = 0
    object SpeedButton_Pessoa: TSpeedButton
      Left = 569
      Top = 44
      Width = 33
      Height = 24
      Hint = 'Permite selecionar nome da cadastro de pessoas do Ensino.'
      Flat = True
      Glyph.Data = {
        
        2617000B0B2402020202020202020202020A030F1E0202020202}
      ParentShowHint = False
      ShowHint = True
      OnClick = SpeedButton_PessoaClick
    end
    object Label_Unidade: TLabel
      Left = 17
      Top = 115
      Width = 124
      Height = 16
      Caption = 'Cargo............................:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label1: TLabel
      Left = 17
      Top = 51
      Width = 124
      Height = 16
      Caption = 'Nome............................:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 17
      Top = 80
      Width = 119
      Height = 16
      Caption = 'Posto/Gradua'#231#227'o...:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 17
      Top = 19
      Width = 124
      Height = 16
      Caption = 'Sigla..............................:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object DBEdit_Descricao: TDBEdit
      Left = 146
      Top = 109
      Width = 575
      Height = 24
      BevelInner = bvSpace
      BevelKind = bkFlat
      BorderStyle = bsNone
      DataField = 'DESCCARGO'
      DataSource = DST_Cadastro
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
    end
    object DBEdit_PostoGraduacao: TDBEdit
      Left = 146
      Top = 74
      Width = 170
      Height = 24
      BevelInner = bvSpace
      BevelKind = bkFlat
      BorderStyle = bsNone
      DataField = 'POSTOGRADUACAO'
      DataSource = DST_Cadastro
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 2
    end
    object DBEdit_Sigla: TDBEdit
      Left = 146
      Top = 13
      Width = 95
      Height = 24
      BevelInner = bvSpace
      BevelKind = bkFlat
      BorderStyle = bsNone
      CharCase = ecUpperCase
      DataField = 'SIGLA'
      DataSource = DST_Cadastro
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object DBEdit_Nome: TDBEdit
      Left = 146
      Top = 44
      Width = 422
      Height = 24
      BevelInner = bvSpace
      BevelKind = bkFlat
      BorderStyle = bsNone
      DataField = 'NOME'
      DataSource = DST_Pessoa
      TabOrder = 3
    end
  end
  object Panel_Grid: TPanel
    Left = 0
    Top = 267
    Width = 870
    Height = 486
    Align = alClient
    BevelInner = bvLowered
    BorderWidth = 4
    Color = 16577515
    TabOrder = 1
    object DBGrid_Campos: TDBGrid
      Left = 6
      Top = 35
      Width = 851
      Height = 445
      Align = alLeft
      Ctl3D = False
      DataSource = DSQ_Consulta
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowSelect, dgAlwaysShowSelection, dgMultiSelect]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clMaroon
      TitleFont.Height = -13
      TitleFont.Name = 'Times New Roman'
      TitleFont.Style = []
      OnTitleClick = DBGrid_CamposTitleClick
      Columns = <
        item
          Expanded = False
          FieldName = 'POSTOGRADUACAO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Posto/Gradua'#231#227'o'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 119
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOME'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'N o m e'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 287
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DESCCARGO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Cargo'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 417
          Visible = True
        end>
    end
    object ToolBar1: TToolBar
      Left = 6
      Top = 6
      Width = 858
      Height = 29
      ButtonHeight = 24
      Caption = 'ToolBar_Selecao'
      TabOrder = 1
      object ComboBox_Posto_Graduacao: TComboBox
        Left = 0
        Top = 2
        Width = 121
        Height = 24
        BevelInner = bvSpace
        BevelKind = bkFlat
        ItemHeight = 16
        TabOrder = 2
        OnChange = ComboBox_Posto_GraduacaoChange
      end
      object Edit_Nome: TEdit
        Left = 121
        Top = 2
        Width = 288
        Height = 24
        BevelInner = bvSpace
        BevelKind = bkFlat
        BorderStyle = bsNone
        CharCase = ecUpperCase
        MaxLength = 12
        TabOrder = 1
        OnChange = ComboBox_Posto_GraduacaoChange
      end
      object Edit_Cargo: TEdit
        Left = 409
        Top = 2
        Width = 419
        Height = 24
        BevelInner = bvSpace
        BevelKind = bkFlat
        BorderStyle = bsNone
        CharCase = ecUpperCase
        MaxLength = 40
        TabOrder = 0
        OnChange = ComboBox_Posto_GraduacaoChange
      end
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 55
    Width = 870
    Height = 63
    Align = alTop
    BevelOuter = bvNone
    BorderStyle = bsSingle
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 2
    object JvGradient2: TJvGradient
      Left = 0
      Top = 0
      Width = 868
      Height = 61
      Align = alClient
      Style = grVertical
      StartColor = 4227200
      EndColor = 16710631
      Steps = 45
    end
    object JvShape1: TJvShape
      Left = 412
      Top = 3
      Width = 114
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Grava: TJvTransparentButton
      Left = 414
      Top = 4
      Width = 112
      Height = 53
      Hint = 'Grava inclus'#227'o ou altera'#231#227'o.'
      BorderWidth = 0
      Caption = 'Gravar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        2F167533146A300F6B2C136C2D1C5A260DB0855F0000}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_GravaClick
    end
    object JvShape2: TJvShape
      Left = 531
      Top = 3
      Width = 114
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Cancel: TJvTransparentButton
      Left = 533
      Top = 4
      Width = 112
      Height = 53
      Hint = 'Cancela a inclus'#227'o ou altera'#231#227'o'
      BorderWidth = 0
      Caption = 'Cancelar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        C3C3C3C3C3C3C3C30000}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_CancelClick
    end
    object JvShape3: TJvShape
      Left = 11
      Top = 3
      Width = 132
      Height = 52
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Inserir: TJvTransparentButton
      Left = 12
      Top = 4
      Width = 130
      Height = 52
      Hint = 'Insere um novo Cargo'
      BorderWidth = 0
      Caption = 'Adicionar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        }
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_InserirClick
    end
    object JvShape5: TJvShape
      Left = 147
      Top = 3
      Width = 132
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Altera: TJvTransparentButton
      Left = 148
      Top = 4
      Width = 130
      Height = 51
      Hint = 'Permite alterar os dados do cargo selecionado.'
      BorderWidth = 0
      Caption = 'Alterar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FCFEFAFEF4EBFEEFE7FAFAF9F9FFF8F3F2FCFCF2FDFDFE000000}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_AlteraClick
    end
    object JvShape8: TJvShape
      Left = 284
      Top = 3
      Width = 114
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Exclui: TJvTransparentButton
      Left = 285
      Top = 4
      Width = 112
      Height = 51
      Hint = 'Exclui o cargo selecionado.'
      BorderWidth = 0
      Caption = 'Excluir'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        7DBFBEBCFFFFFFFDFEFD}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_ExcluiClick
    end
  end
  object Panel_Botoes: TPanel
    Left = 0
    Top = 0
    Width = 870
    Height = 55
    Align = alTop
    BevelOuter = bvNone
    Color = 4227200
    TabOrder = 3
    object JvShape6: TJvShape
      Left = 25
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Retornar: TJvTransparentButton
      Left = 26
      Top = 3
      Width = 71
      Height = 49
      Hint = 'Retornar para Janela anterior.'
      BorderWidth = 0
      Caption = 'Retornar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_RetornarClick
    end
    object JvShape7: TJvShape
      Left = 105
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Windows: TJvTransparentButton
      Left = 106
      Top = 3
      Width = 71
      Height = 49
      Hint = 'Sai do sistema e retorna para sistema operacional.'
      BorderWidth = 0
      Caption = 'Windows'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_WindowsClick
    end
  end
  object DST_Cadastro: TDataSource
    DataSet = Table_Cadastro
    Left = 447
    Top = 359
  end
  object DSQ_Consulta: TDataSource
    DataSet = Query_Consulta
    Left = 391
    Top = 359
  end
  object Table_Cadastro: TIBTable
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    OnPostError = Table_CadastroPostError
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'IDCARGO'
        Attributes = [faRequired]
        DataType = ftSmallint
      end
      item
        Name = 'IDENTIDADE'
        Attributes = [faRequired, faFixed]
        DataType = ftString
        Size = 20
      end
      item
        Name = 'SIGLA'
        Attributes = [faRequired]
        DataType = ftString
        Size = 5
      end
      item
        Name = 'POSTOGRADUACAO'
        Attributes = [faRequired, faFixed]
        DataType = ftString
        Size = 25
      end
      item
        Name = 'DESCCARGO'
        DataType = ftString
        Size = 50
      end>
    IndexDefs = <
      item
        Name = 'IDCARGO_PK'
        Fields = 'IDCARGO'
        Options = [ixUnique]
      end
      item
        Name = 'CARGO_IDENTIDADE_FK'
        Fields = 'IDENTIDADE'
      end>
    IndexFieldNames = 'IDCARGO'
    MasterFields = 'IDCARGO'
    MasterSource = DSQ_Consulta
    StoreDefs = True
    TableName = 'CARGO'
    Left = 446
    Top = 327
    object Table_CadastroIDCARGO: TSmallintField
      FieldName = 'IDCARGO'
      Required = True
    end
    object Table_CadastroPOSTOGRADUACAO: TIBStringField
      FieldName = 'POSTOGRADUACAO'
      Required = True
      FixedChar = True
      Size = 25
    end
    object Table_CadastroSIGLA: TIBStringField
      FieldName = 'SIGLA'
      Size = 5
    end
    object Table_CadastroIDENTIDADE: TIBStringField
      FieldName = 'IDENTIDADE'
      Required = True
    end
    object Table_CadastroDESCCARGO: TIBStringField
      FieldName = 'DESCCARGO'
      Size = 50
    end
  end
  object Query_Consulta: TIBQuery
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    BufferChunks = 1000
    CachedUpdates = True
    SQL.Strings = (
      'Select ca.IdCargo, ca.DescCargo, ca.PostoGraduacao, ca.nome'
      'from Cargo_view ca')
    Left = 393
    Top = 326
    object Query_ConsultaIDCARGO: TSmallintField
      FieldName = 'IDCARGO'
      Origin = 'CARGO_VIEW.IDCARGO'
    end
    object Query_ConsultaDESCCARGO: TIBStringField
      FieldName = 'DESCCARGO'
      Origin = 'CARGO_VIEW.DESCCARGO'
      Size = 50
    end
    object Query_ConsultaPOSTOGRADUACAO: TIBStringField
      FieldName = 'POSTOGRADUACAO'
      Origin = 'CARGO_VIEW.POSTOGRADUACAO'
      FixedChar = True
      Size = 25
    end
    object Query_ConsultaNOME: TIBStringField
      FieldName = 'NOME'
      Origin = 'CARGO_VIEW.NOME'
      Size = 100
    end
  end
  object IBTable_Pessoa: TIBTable
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'IDENTIDADE'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'IDENTIDADE_OLD'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'NOME'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'NOMEGUERRA'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'POSTOGRADUACAO'
        DataType = ftString
        Size = 25
      end
      item
        Name = 'ULTIMAPROMOCAO'
        DataType = ftDateTime
      end
      item
        Name = 'SEXO'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'DATA_NASCIMENTO'
        DataType = ftDateTime
      end
      item
        Name = 'NOMEPAIS'
        DataType = ftString
        Size = 12
      end
      item
        Name = 'EMAIL'
        DataType = ftString
        Size = 150
      end
      item
        Name = 'CPF_CIC'
        DataType = ftString
        Size = 12
      end
      item
        Name = 'SARAM'
        DataType = ftString
        Size = 7
      end
      item
        Name = 'QUADRO'
        DataType = ftString
        Size = 8
      end
      item
        Name = 'ESPECIALIDADE'
        DataType = ftString
        Size = 8
      end
      item
        Name = 'ORGAOEMISSOR'
        DataType = ftString
        Size = 10
      end
      item
        Name = 'ESTADO_CIVIL'
        DataType = ftString
        Size = 10
      end
      item
        Name = 'NATURALIDADE_CIDADE'
        DataType = ftString
        Size = 40
      end
      item
        Name = 'NATURALIDADE_UF'
        DataType = ftString
        Size = 2
      end
      item
        Name = 'NOME_PAI'
        DataType = ftString
        Size = 50
      end
      item
        Name = 'NOME_MAE'
        DataType = ftString
        Size = 50
      end
      item
        Name = 'IDUNIDADE'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'IDSUBUNIDADE'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'QUADROESP'
        DataType = ftString
        Size = 16
      end
      item
        Name = 'SIPAER'
        DataType = ftString
        Size = 7
      end>
    IndexDefs = <
      item
        Name = 'PESSOA_PK'
        Fields = 'IDENTIDADE'
        Options = [ixUnique]
      end
      item
        Name = 'PESSOA_IDX_ANTIGUIDADE'
        Fields = 'POSTOGRADUACAO;ULTIMAPROMOCAO'
      end
      item
        Name = 'PESSOA_NOMEGUERRA_IDX'
        Fields = 'NOMEGUERRA'
      end
      item
        Name = 'PESSOA_IDX_NOME'
        Fields = 'NOME'
      end>
    IndexFieldNames = 'IDENTIDADE'
    MasterFields = 'IDENTIDADE'
    MasterSource = DST_Cadastro
    StoreDefs = True
    TableName = 'PESSOA'
    Left = 542
    Top = 327
    object IBTable_PessoaIDENTIDADE: TIBStringField
      FieldName = 'IDENTIDADE'
      Required = True
    end
    object IBTable_PessoaPOSTOGRADUACAO: TIBStringField
      FieldName = 'POSTOGRADUACAO'
      FixedChar = True
      Size = 25
    end
    object IBTable_PessoaNOME: TIBStringField
      FieldName = 'NOME'
      Size = 100
    end
    object IBTable_PessoaNOMEGUERRA: TIBStringField
      FieldName = 'NOMEGUERRA'
    end
  end
  object DST_Pessoa: TDataSource
    DataSet = IBTable_Pessoa
    Left = 543
    Top = 359
  end
end

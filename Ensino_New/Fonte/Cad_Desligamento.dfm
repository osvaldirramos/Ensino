object Form_Tab_Desligamento: TForm_Tab_Desligamento
  Left = 72
  Top = 125
  Width = 1206
  Height = 565
  Caption = 'Ensino --> Motivo de Desligamento'
  Color = 16577515
  Ctl3D = False
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  OnActivate = FormActivate
  PixelsPerInch = 96
  TextHeight = 16
  object Panel_Entrada_Dados: TPanel
    Left = 0
    Top = 118
    Width = 1198
    Height = 78
    Align = alTop
    Color = 16577515
    TabOrder = 0
    object Label_Descricao: TLabel
      Left = 17
      Top = 48
      Width = 68
      Height = 16
      Caption = 'Descri'#231#227'o.:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label_Motivo: TLabel
      Left = 17
      Top = 15
      Width = 68
      Height = 16
      Caption = 'Alin'#234'a.........:'
      FocusControl = DBEdit_Descricao
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object JvEnterAsTab1: TJvEnterAsTab
      Left = 368
      Top = 1
      Width = 28
      Height = 28
    end
    object DBEdit_Motivo: TDBEdit
      Left = 92
      Top = 11
      Width = 85
      Height = 22
      DataField = 'ALINEA'
      DataSource = DST_Cadastro
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object DBEdit_Descricao: TDBEdit
      Left = 93
      Top = 44
      Width = 860
      Height = 22
      DataField = 'DESCRICAO'
      DataSource = DST_Cadastro
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
    end
  end
  object Panel_Grid: TPanel
    Left = 0
    Top = 196
    Width = 1198
    Height = 338
    Align = alClient
    BevelInner = bvLowered
    BorderWidth = 4
    Color = 16577515
    TabOrder = 1
    object DBGrid_Campos: TDBGrid
      Left = 6
      Top = 30
      Width = 1180
      Height = 302
      Align = alLeft
      DataSource = DSQ_Consulta
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowSelect, dgAlwaysShowSelection, dgMultiSelect]
      ParentFont = False
      TabOrder = 0
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clMaroon
      TitleFont.Height = -13
      TitleFont.Name = 'Times New Roman'
      TitleFont.Style = []
      OnTitleClick = DBGrid_CamposTitleClick
      Columns = <
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'ALINEA'
          Title.Alignment = taCenter
          Title.Caption = 'Alin'#234'a'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DESCRICAO'
          Title.Alignment = taCenter
          Title.Caption = 'Descri'#231#227'o'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Visible = True
        end>
    end
    object ToolBar1: TToolBar
      Left = 6
      Top = 6
      Width = 1186
      Height = 24
      Caption = 'ToolBar_Selecao'
      EdgeBorders = []
      EdgeInner = esNone
      EdgeOuter = esNone
      TabOrder = 1
      object Edit_Motivo: TEdit
        Left = 0
        Top = 2
        Width = 43
        Height = 22
        MaxLength = 1
        TabOrder = 1
        OnChange = Edit_MotivoChange
      end
      object Edit_Descricao: TEdit
        Left = 43
        Top = 2
        Width = 1116
        Height = 22
        MaxLength = 5
        TabOrder = 0
        OnChange = Edit_DescricaoChange
      end
    end
  end
  object Panel_Botoes: TPanel
    Left = 0
    Top = 0
    Width = 1198
    Height = 55
    Align = alTop
    BevelOuter = bvNone
    Color = 16577515
    TabOrder = 2
    object JvShape4: TJvShape
      Left = 9
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Atualizar: TJvTransparentButton
      Left = 10
      Top = 3
      Width = 71
      Height = 51
      Hint = 'Atualiza a lista de quadros'
      BorderWidth = 0
      Caption = 'Atualizar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        7C2CBE7D2CC3C3C3C3C3C3C3C3C3C3C3C300}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_AtualizarClick
    end
    object JvShape6: TJvShape
      Left = 97
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Retornar: TJvTransparentButton
      Left = 98
      Top = 3
      Width = 71
      Height = 49
      Hint = 'Retornar para Janela anterior.'
      BorderWidth = 0
      Caption = 'Retornar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_RetornarClick
    end
    object JvShape7: TJvShape
      Left = 177
      Top = 2
      Width = 73
      Height = 51
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Windows: TJvTransparentButton
      Left = 178
      Top = 3
      Width = 71
      Height = 49
      Hint = 'Sai do sistema e retorna para sistema operacional.'
      BorderWidth = 0
      Caption = 'Windows'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_WindowsClick
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 55
    Width = 1198
    Height = 63
    Align = alTop
    BevelOuter = bvNone
    BorderStyle = bsSingle
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 3
    object JvGradient1: TJvGradient
      Left = 0
      Top = 0
      Width = 1196
      Height = 61
      Align = alClient
      Style = grVertical
      StartColor = 4227200
      EndColor = 16710631
      Steps = 45
    end
    object JvShape1: TJvShape
      Left = 294
      Top = 3
      Width = 78
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Grava: TJvTransparentButton
      Left = 296
      Top = 4
      Width = 76
      Height = 53
      Hint = 'Grava inclus'#227'o ou altera'#231#227'o.'
      BorderWidth = 0
      Caption = 'Gravar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        2F167533146A300F6B2C136C2D1C5A260DB0855F0000}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_GravaClick
    end
    object JvShape2: TJvShape
      Left = 377
      Top = 3
      Width = 78
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Cancel: TJvTransparentButton
      Left = 379
      Top = 4
      Width = 76
      Height = 53
      Hint = 'Cancela a inclus'#227'o ou altera'#231#227'o'
      BorderWidth = 0
      Caption = 'Cancelar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        C3C3C3C3C3C3C3C30000}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_CancelClick
    end
    object JvShape3: TJvShape
      Left = 11
      Top = 3
      Width = 96
      Height = 52
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Inserir: TJvTransparentButton
      Left = 12
      Top = 4
      Width = 94
      Height = 52
      Hint = 'Insere um novo Motivo de Desligamento.'
      BorderWidth = 0
      Caption = 'Adicionar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        }
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_InserirClick
    end
    object JvShape5: TJvShape
      Left = 111
      Top = 3
      Width = 96
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Altera: TJvTransparentButton
      Left = 112
      Top = 4
      Width = 94
      Height = 51
      Hint = 'Permite alterar os dados do motivo selecionado.'
      BorderWidth = 0
      Caption = 'Alterar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FCFEFAFEF4EBFEEFE7FAFAF9F9FFF8F3F2FCFCF2FDFDFE000000}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_AlteraClick
    end
    object JvShape8: TJvShape
      Left = 211
      Top = 3
      Width = 78
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Exclui: TJvTransparentButton
      Left = 212
      Top = 4
      Width = 76
      Height = 51
      Hint = 'Exclui o motivo selecionado.'
      BorderWidth = 0
      Caption = 'Excluir'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        7DBFBEBCFFFFFFFDFEFD}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_ExcluiClick
    end
  end
  object DST_Cadastro: TDataSource
    DataSet = Table_Cadastro
    Left = 319
    Top = 230
  end
  object DSQ_Consulta: TDataSource
    DataSet = Query_Consulta
    Left = 263
    Top = 230
  end
  object Query_Consulta: TIBQuery
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    BufferChunks = 1000
    CachedUpdates = True
    SQL.Strings = (
      'Select MotivoDeslig, Descricao, Alinea, Versao'
      'From Desligamento'
      'where versao = 2'
      'Order by MotivoDeslig')
    Left = 264
    Top = 198
    object Query_ConsultaMOTIVODESLIG: TIBStringField
      FieldName = 'MOTIVODESLIG'
      Origin = 'DESLIGAMENTO.MOTIVODESLIG'
      Required = True
      FixedChar = True
      Size = 1
    end
    object Query_ConsultaDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Origin = 'DESLIGAMENTO.DESCRICAO'
      Required = True
      Size = 159
    end
    object Query_ConsultaALINEA: TIBStringField
      FieldName = 'ALINEA'
      Origin = 'DESLIGAMENTO.ALINEA'
      FixedChar = True
      Size = 1
    end
    object Query_ConsultaVERSAO: TIBStringField
      FieldName = 'VERSAO'
      Origin = 'DESLIGAMENTO.VERSAO'
      FixedChar = True
      Size = 1
    end
  end
  object Table_Cadastro: TIBTable
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    BufferChunks = 1000
    CachedUpdates = False
    FieldDefs = <
      item
        Name = 'MOTIVODESLIG'
        Attributes = [faRequired, faFixed]
        DataType = ftString
        Size = 1
      end
      item
        Name = 'ALINEA'
        Attributes = [faFixed]
        DataType = ftString
        Size = 1
      end
      item
        Name = 'VERSAO'
        Attributes = [faFixed]
        DataType = ftString
        Size = 1
      end
      item
        Name = 'DESCRICAO'
        Attributes = [faRequired]
        DataType = ftString
        Size = 159
      end>
    IndexDefs = <
      item
        Name = 'MOTIVODESLIG_PK'
        Fields = 'MOTIVODESLIG'
        Options = [ixUnique]
      end>
    IndexFieldNames = 'MOTIVODESLIG'
    MasterFields = 'MOTIVODESLIG'
    MasterSource = DSQ_Consulta
    StoreDefs = True
    TableName = 'DESLIGAMENTO'
    Left = 320
    Top = 198
    object Table_CadastroMOTIVODESLIG: TIBStringField
      FieldName = 'MOTIVODESLIG'
      Size = 1
    end
    object Table_CadastroDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Size = 159
    end
    object Table_CadastroALINEA: TIBStringField
      FieldName = 'ALINEA'
      Size = 1
    end
    object Table_CadastroVERSAO: TIBStringField
      FieldName = 'VERSAO'
      Size = 1
    end
  end
end

object Form_AlunoComProvasDeRecuperacao: TForm_AlunoComProvasDeRecuperacao
  Left = 423
  Top = 112
  Width = 907
  Height = 870
  Caption = 'Consulta de Aluno que fizeram prova de recupera'#231#227'o'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object DBGrid_Curso: TDBGrid
    Left = 0
    Top = 59
    Width = 899
    Height = 780
    Align = alClient
    Color = clCream
    DataSource = DSQ_MontaCurso
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgColLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    TabOrder = 0
    TitleFont.Charset = ANSI_CHARSET
    TitleFont.Color = clGray
    TitleFont.Height = -17
    TitleFont.Name = 'Times New Roman'
    TitleFont.Style = [fsItalic]
    Columns = <
      item
        Expanded = False
        FieldName = 'NOMECOMPLETO'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Caption = 'Nome do Aluno'
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 715
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'CODPROVA'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Caption = 'C'#243'd. Prova'
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Width = 78
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'NOTA'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        Title.Alignment = taCenter
        Title.Caption = 'Nota'
        Title.Color = 16311249
        Title.Font.Charset = ANSI_CHARSET
        Title.Font.Color = clNavy
        Title.Font.Height = -13
        Title.Font.Name = 'Arial'
        Title.Font.Style = []
        Visible = True
      end>
  end
  object Panel_Botoes: TPanel
    Left = 0
    Top = 0
    Width = 899
    Height = 59
    Align = alTop
    BevelOuter = bvNone
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 1
    object JvGradient_Tela01: TJvGradient
      Left = 0
      Top = 0
      Width = 899
      Height = 59
      Align = alClient
      Style = grVertical
      StartColor = 4227200
      EndColor = 16777200
      Steps = 58
    end
    object JvShape9: TJvShape
      Left = 121
      Top = 3
      Width = 66
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Fechar: TJvTransparentButton
      Left = 122
      Top = 5
      Width = 64
      Height = 51
      Hint = 'Retornar para Menu Principal'
      BorderWidth = 0
      Caption = 'Retornar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_FecharClick
    end
    object JvShape10: TJvShape
      Left = 201
      Top = 3
      Width = 69
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Windows: TJvTransparentButton
      Left = 202
      Top = 5
      Width = 67
      Height = 51
      Hint = 'Sai do sistema e retorna para sistema operacional.'
      BorderWidth = 0
      Caption = 'Windows'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_WindowsClick
    end
    object JvShape1: TJvShape
      Left = 33
      Top = 3
      Width = 66
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Excel: TJvTransparentButton
      Left = 34
      Top = 5
      Width = 64
      Height = 51
      Hint = 'Tratamento das informacoes para Excel'
      BorderWidth = 0
      Caption = 'Excel'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_ExcelClick
    end
    object Memo1: TMemo
      Left = 602
      Top = 7
      Width = 245
      Height = 32
      BevelInner = bvLowered
      Color = 5855743
      Ctl3D = False
      Lines.Strings = (
        'Informa'#231#227'o RESERVADA de acordo com a '
        'LEI N'#186' 12.527, DE 18 DE NOVEMBRO DE 2011')
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
    end
  end
  object Query_MontaRelacao: TIBQuery
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    BufferChunks = 1000
    CachedUpdates = True
    SQL.Strings = (
      
        'Select distinct n.IdCurso, n.IdTurma, n.Identidade, a.NomeComple' +
        'to, n.CodProva, Nota'
      'From Nota_Aluno_View n, Alunos_Turma_View a'
      'Where  n.Identidade = a.Identidade'
      
        'and  ( ReferenciaAvaliacao like '#39'%2'#39' or ReferenciaAvaliacao like' +
        ' '#39'%F'#39' or ReferenciaAvaliacao like '#39'%R'#39')'
      'Order By a.NomeCompleto, n.CodProva')
    Left = 379
    Top = 162
    object Query_MontaRelacaoIDENTIDADE: TIBStringField
      FieldName = 'IDENTIDADE'
      Origin = 'NOTAALUNO_VIEW.IDENTIDADE'
      FixedChar = True
    end
    object Query_MontaRelacaoNOMECOMPLETO: TIBStringField
      FieldName = 'NOMECOMPLETO'
      Origin = 'ALUNOS_TURMA_VIEW.NOMECOMPLETO'
      Size = 125
    end
    object Query_MontaRelacaoCODPROVA: TIBStringField
      FieldName = 'CODPROVA'
      Origin = 'NOTAALUNO_VIEW.CODPROVA'
      Size = 4
    end
    object Query_MontaRelacaoNOTA: TIBBCDField
      FieldName = 'NOTA'
      Origin = 'NOTAALUNO_VIEW.NOTA'
      DisplayFormat = '#0.00'
      Precision = 9
      Size = 2
    end
  end
  object DSQ_MontaCurso: TDataSource
    DataSet = Query_MontaRelacao
    Left = 424
    Top = 160
  end
end

object Form_EstatisticaAplicacaoProva: TForm_EstatisticaAplicacaoProva
  Left = 217
  Top = 128
  Width = 924
  Height = 380
  Caption = 'Estat'#237'stica Aplica'#231#227'o de Prova'
  Color = 16777200
  Ctl3D = False
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object Panel_Botoes: TPanel
    Left = 0
    Top = 0
    Width = 916
    Height = 62
    Align = alTop
    BevelOuter = bvNone
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 0
    object JvGradient3: TJvGradient
      Left = 0
      Top = 0
      Width = 916
      Height = 62
      Align = alClient
      Style = grVertical
      StartColor = 4227200
      EndColor = 16777200
      Steps = 62
    end
    object JvShape9: TJvShape
      Left = 121
      Top = 0
      Width = 66
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Fechar: TJvTransparentButton
      Left = 122
      Top = 2
      Width = 64
      Height = 51
      Hint = 'Retornar para Menu Principal'
      BorderWidth = 0
      Caption = 'Retornar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_FecharClick
    end
    object JvShape10: TJvShape
      Left = 201
      Top = 0
      Width = 69
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Windows: TJvTransparentButton
      Left = 202
      Top = 2
      Width = 67
      Height = 51
      Hint = 'Sai do sistema e retorna para sistema operacional.'
      BorderWidth = 0
      Caption = 'Windows'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        290097390600A13C0800A33D0700A2420200A5430500FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_WindowsClick
    end
    object JvShape1: TJvShape
      Left = 33
      Top = 0
      Width = 66
      Height = 53
      Brush.Color = 16378331
      Pen.Color = 33023
    end
    object JvTransparentButton_Excel: TJvTransparentButton
      Left = 34
      Top = 2
      Width = 64
      Height = 51
      Hint = 'Tratamento das informacoes para Excel'
      BorderWidth = 0
      Caption = 'Excel'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      FrameStyle = fsDark
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TextAlign = ttaBottom
      OnClick = JvTransparentButton_ExcelClick
    end
    object Memo1: TMemo
      Left = 354
      Top = 7
      Width = 245
      Height = 41
      BevelInner = bvLowered
      Color = 5855743
      Ctl3D = False
      Lines.Strings = (
        'Informa'#231#227'o RESERVADA de acordo com a '
        'LEI N'#186' 12.527, DE 18 DE NOVEMBRO DE 2011')
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
    end
  end
  object JvPanel1: TJvPanel
    Left = 0
    Top = 62
    Width = 916
    Height = 287
    Align = alClient
    BevelOuter = bvNone
    Color = 16777200
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 1
    MultiLine = False
    object Panel_Teorica: TPanel
      Left = 6
      Top = 30
      Width = 259
      Height = 25
      Alignment = taLeftJustify
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Prova Te'#243'rica'
      Color = 16769217
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
    object Panel_Pratica: TPanel
      Left = 6
      Top = 54
      Width = 259
      Height = 25
      Alignment = taLeftJustify
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Prova Pratica'
      Color = 16773087
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
    end
    object Panel_2Chamada: TPanel
      Left = 6
      Top = 78
      Width = 259
      Height = 25
      Alignment = taLeftJustify
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Prova de 2'#170' chamada'
      Color = 16769217
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
    end
    object Panel_RecTeorica: TPanel
      Left = 6
      Top = 102
      Width = 259
      Height = 25
      Alignment = taLeftJustify
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Prova de recupera'#231#227'o Te'#243'rica'
      Color = 16773087
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
    end
    object Panel_RecPratica: TPanel
      Left = 6
      Top = 126
      Width = 259
      Height = 25
      Alignment = taLeftJustify
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Prova de recupera'#231#227'o Pr'#225'tica'
      Color = 16769217
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
    end
    object Panel_Diagnostica: TPanel
      Left = 6
      Top = 150
      Width = 259
      Height = 25
      Alignment = taLeftJustify
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Prova diagn'#243'stica:'
      Color = 16773087
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
    end
    object Panel_ProvaTeorica: TPanel
      Left = 264
      Top = 30
      Width = 80
      Height = 25
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = '1200'
      Color = 16769217
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
    end
    object Panel_ProvaPratica: TPanel
      Left = 264
      Top = 54
      Width = 80
      Height = 25
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = '1200'
      Color = 16773087
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 7
    end
    object Panel_ProvaSegunda: TPanel
      Left = 264
      Top = 78
      Width = 80
      Height = 25
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = '1200'
      Color = 16769217
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 8
    end
    object Panel_ProvaRecTeo: TPanel
      Left = 264
      Top = 102
      Width = 80
      Height = 25
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = '1200'
      Color = 16773087
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 9
    end
    object Panel_ProvaRecPra: TPanel
      Left = 264
      Top = 126
      Width = 80
      Height = 25
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = '1200'
      Color = 16769217
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 10
    end
    object Panel_ProvaDiagnostica: TPanel
      Left = 264
      Top = 150
      Width = 80
      Height = 25
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = '1200'
      Color = 16773087
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 11
    end
    object Panel1_TotalProvas: TPanel
      Left = 6
      Top = 180
      Width = 259
      Height = 25
      Alignment = taLeftJustify
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Total de Provas'
      Color = 16757568
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 12
    end
    object Panel_TotalProvas: TPanel
      Left = 264
      Top = 180
      Width = 80
      Height = 25
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = '1200'
      Color = 16757568
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 13
    end
    object Panel2: TPanel
      Left = 6
      Top = 1
      Width = 259
      Height = 24
      Alignment = taLeftJustify
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Selecione o ano'
      Color = 16769217
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 14
    end
    object ComboBox_Ano: TComboBox
      Left = 263
      Top = 1
      Width = 80
      Height = 24
      BevelInner = bvSpace
      BevelKind = bkFlat
      Ctl3D = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ItemHeight = 16
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 15
      OnChange = ComboBox_AnoChange
      Items.Strings = (
        '2010'
        '2011'
        '2012'
        '2013'
        '2014'
        '2015'
        '2016'
        '2017'
        '2018'
        '2019'
        '220'
        '2021'
        '2022'
        '2023'
        '2024'
        '2025'
        '2026'
        '2027'
        '2028'
        '2029'
        '2030'
        '2031')
    end
    object DBGrid_Curso: TDBGrid
      Left = 382
      Top = 0
      Width = 534
      Height = 287
      Align = alRight
      Color = 15921906
      Ctl3D = False
      DataSource = DSQ_MontaRelacao
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Options = [dgTitles, dgColLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 16
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clGray
      TitleFont.Height = -17
      TitleFont.Name = 'Times New Roman'
      TitleFont.Style = [fsItalic]
      Columns = <
        item
          Alignment = taCenter
          Color = 12703487
          Expanded = False
          FieldName = 'CODCURSO'
          Title.Alignment = taCenter
          Title.Caption = 'Cod. Curso'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 121
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 12703487
          Expanded = False
          FieldName = 'TURMA'
          Title.Alignment = taCenter
          Title.Caption = 'Turma'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 12703487
          Expanded = False
          FieldName = 'CODPROVA'
          Title.Alignment = taCenter
          Title.Caption = 'C'#243'd Prova'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Width = 71
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 12703487
          Expanded = False
          FieldName = 'QDEALUNOS'
          Title.Alignment = taCenter
          Title.Caption = 'Qde Alunos'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Visible = True
        end
        item
          Alignment = taCenter
          Color = 12703487
          Expanded = False
          FieldName = 'DATAAPLICACAO'
          Title.Alignment = taCenter
          Title.Caption = 'Data Aplica'#231#227'o'
          Title.Color = 16311249
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -13
          Title.Font.Name = 'Arial'
          Title.Font.Style = []
          Visible = True
        end>
    end
    object Panel1_TotalAlunos: TPanel
      Left = 6
      Top = 206
      Width = 259
      Height = 25
      Alignment = taLeftJustify
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Total de Alunos'
      Color = 16753690
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 17
    end
    object Panel_TotalAlunos: TPanel
      Left = 264
      Top = 206
      Width = 80
      Height = 25
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = '1200'
      Color = 16753690
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 18
    end
    object Panel_Total: TPanel
      Left = 6
      Top = 238
      Width = 259
      Height = 25
      Alignment = taLeftJustify
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = 'Total de Provas por Alunos'
      Color = 14780160
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 19
    end
    object Panel_TotalProvasPorAlunos: TPanel
      Left = 264
      Top = 238
      Width = 80
      Height = 25
      BevelOuter = bvNone
      BorderStyle = bsSingle
      Caption = '1200'
      Color = 14780160
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 20
    end
  end
  object Query_MontaRelacao: TIBQuery
    Database = DM.IBDatabase_Ensino
    Transaction = DM.IBTransaction_Ensino
    BufferChunks = 1000
    CachedUpdates = True
    SQL.Strings = (
      
        'select Tu.codcurso,  Tu.turma,  Te.CodProva, tu.QdeAlunos, Te.Da' +
        'taAplicacao'
      ' From Testes_View Te,  Turmas_view Tu'
      ' Where Te.IdCurso = Tu.idcurso'
      ' and   Te.IdTurma = Tu.IdTurma'
      
        ' and   Te.dataaplicacao between '#39'01.01.2016 00:00'#39' and '#39'31.12.20' +
        '16 00:00'#39
      ' and   ( Te.referenciaAvaliacao Like '#39'%T'#39' or'
      '         Te.referenciaAvaliacao Like '#39'%P'#39' or'
      '         Te.referenciaAvaliacao Like '#39'%2'#39' or'
      '         Te.referenciaAvaliacao Like '#39'%F'#39' or'
      '         Te.referenciaAvaliacao Like '#39'%R'#39' or'
      '         Te.referenciaAvaliacao Like '#39'%D'#39
      '        )'
      'Order By Te.DataAplicacao')
    Left = 427
    Top = 194
    object Query_MontaRelacaoCODCURSO: TIBStringField
      FieldName = 'CODCURSO'
      Origin = 'TURMA_TODAS_VIEW.CODCURSO'
      Size = 10
    end
    object Query_MontaRelacaoTURMA: TIBStringField
      FieldName = 'TURMA'
      Origin = 'TURMA_TODAS_VIEW.TURMA'
      Size = 15
    end
    object Query_MontaRelacaoCODPROVA: TIBStringField
      FieldName = 'CODPROVA'
      Origin = 'TESTE_VIEW.CODPROVA'
      Size = 4
    end
    object Query_MontaRelacaoQDEALUNOS: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'QDEALUNOS'
      Origin = 'TURMA_TODAS_VIEW.QDEALUNOS'
      ReadOnly = True
    end
    object Query_MontaRelacaoDATAAPLICACAO: TDateTimeField
      FieldName = 'DATAAPLICACAO'
      Origin = 'TESTE_VIEW.DATAAPLICACAO'
    end
  end
  object DSQ_MontaRelacao: TDataSource
    DataSet = Query_MontaRelacao
    Left = 472
    Top = 192
  end
end

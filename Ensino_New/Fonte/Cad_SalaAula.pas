...
type
  TForm_CadSalaAula = class(TForm)
    ...
    procedure DBGrid_SalaAulaDrawColumnCell(Sender: TObject;
...
procedure TForm_CadSalaAula.DBGrid_SalaAulaDrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
   if IBQuery_SalaAula.FieldByName('Situacao').AsString = 'INDISPONÍVEL' then
   begin
      GroupBox_Disponibilidades.Visible:= True;
      DBGrid_SalaAula.Canvas.Brush.Color:= clYellow;
      DBGrid_SalaAula.Canvas.Font.Color:= clBlack;
   end
   else
   begin
      GroupBox_Disponibilidades.Visible:= False;
      DBGrid_SalaAula.Canvas.Brush.Color:= clCream;
      DBGrid_SalaAula.Canvas.Font.Color:= clBlack;
   end;

  if gdSelected in State then
    DBGrid_SalaAula.Canvas.Brush.Color := clSkyBlue;

   DBGrid_SalaAula.Canvas.FillRect(Rect);
   DBGrid_SalaAula.DefaultDrawColumnCell(Rect, DataCol, Column, State);
end;
